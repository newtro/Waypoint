...
export function verifyPackagedVoice(resources:string):void{
  ...
  const executable=path.resolve(resources,'../MacOS/Waypoint'),archive=path.join(resources,'app.asar'),environment={ELECTRON_RUN_AS_NODE:'1',PATH:process.env.PATH??''},probe=execFileSync(executable,['-e',`const s=require(${JSON.stringify(`${archive}/node_modules/sherpa-onnx-node`)});if(!s.OfflineTts)process.exit(2)`],{encoding:'utf8',timeout:10_000,maxBuffer:256*1024,env:environment});if(probe.trim())throw new Error('Packaged sherpa runtime probe emitted unexpected output')
  const modelRoot=path.join(resources,'fast-local/kitten'),benchmark=execFileSync(executable,['-e',`const p=require('node:path'),s=require(${JSON.stringify(`${archive}/node_modules/sherpa-onnx-node`)}),r=${JSON.stringify(modelRoot)};(async()=>{const t=await s.OfflineTts.createAsync({model:{kitten:{model:p.join(r,'model.fp16.onnx'),voices:p.join(r,'voices.bin'),tokens:p.join(r,'tokens.txt'),dataDir:p.join(r,'espeak-ng-data')}},maxNumSentences:1,numThreads:2,provider:'cpu'}),started=performance.now(),out=await t.generateAsync({text:'Waypoint is ready to help. This is the packaged Fast Local voice benchmark.',sid:0,speed:1,enableExternalBuffer:false}),elapsed=performance.now()-started;if(!out.samples.length||out.sampleRate!==24000||elapsed>1000)process.exit(3);console.log(JSON.stringify({firstPlayableAudioMs:elapsed,samples:out.samples.length,sampleRate:out.sampleRate}))})()`],{encoding:'utf8',timeout:20_000,maxBuffer:256*1024,env:environment});const measured=JSON.parse(benchmark.trim())as{firstPlayableAudioMs:number};if(!Number.isFinite(measured.firstPlayableAudioMs)||measured.firstPlayableAudioMs>1000)throw new Error('Packaged Fast Local latency gate failed')
