...
export function verifyPackagedRuntime(archive:string):void{
  ...
  const applicationTests=entries.filter((entry)=>/^\/dist-electron\/electron\/.*\.test\.js$/i.test(entry))
  if(applicationTests.length)throw new Error(`Compiled application tests must not be shipped in the release archive:\n${applicationTests.join('\n')}`)
  const bundledClaude=entries.filter((entry)=>/\/node_modules\/@anthropic-ai\/claude-agent-sdk-[^/]+\/claude(?:\.exe)?$/i.test(entry))
  if(bundledClaude.length)throw new Error(`Waypoint must use the user's discovered Claude CLI; bundled native Claude payload found:\n${bundledClaude.join('\n')}`)
}
...
export type PackagedPerformanceProfile='target'|'hosted-ci'
// Windows process isolation adds a small, repeatable startup tax while keeping native voice work off Electron's main process.
export function packagedFastLocalLimit(profile:PackagedPerformanceProfile):number{return profile==='target'?1100:3000}
export function validPackagedFastLocalMetric(measured:{firstPlayableAudioMs:number;samples:number;sampleRate:number},profile:PackagedPerformanceProfile='target'):boolean{
  return Number.isFinite(measured.firstPlayableAudioMs)&&measured.firstPlayableAudioMs>=0&&measured.firstPlayableAudioMs<=packagedFastLocalLimit(profile)&&Number.isInteger(measured.samples)&&measured.samples>0&&measured.sampleRate===24000
}

export const packagedFastLocalResponse='Waypoint is ready to help. This representative response verifies that later speech remains queued.'
export const packagedFastLocalOpening=(()=>{const opening=fastSpeechSegments(packagedFastLocalResponse)[0];if(!opening)throw new Error('Packaged Fast Local benchmark has no opening segment');return opening})()
...
function verifyPackagedFastLocal(resources:string,executable:string,archive:string):void{
  ...
  const modelRoot=path.join(resources,'fast-local/kitten'),worker=path.join(archive,'dist-electron/electron/core/fast-local-speech-worker.js'),benchmark=execFileSync(executable,['-e',`const{fork}=require('node:child_process'),{writeSync}=require('node:fs'),child=fork(${JSON.stringify(worker)},[${JSON.stringify(modelRoot)}],{stdio:['ignore','ignore','ignore','ipc'],serialization:'advanced',execArgv:[],env:{ELECTRON_RUN_AS_NODE:'1',PATH:process.env.PATH||'',LANG:process.env.LANG||'en_US.UTF-8'}});let settled=false,started;const finish=(code,value)=>{if(settled)return;settled=true;clearTimeout(timer);if(value)writeSync(1,JSON.stringify(value));if(child.connected)child.disconnect();if(!child.killed)child.kill('SIGTERM');process.exit(code)},timer=setTimeout(()=>finish(4),15000);child.on('message',value=>{if(value?.type==='ready'){if(value.ready!==true)return finish(3);started=performance.now();return child.send({type:'speak',id:1,text:${JSON.stringify(packagedFastLocalOpening)}})}if(value?.type==='audio'&&value.id===1&&value.samples instanceof Float32Array&&value.samples.length&&value.sampleRate===24000&&started!==undefined)return finish(0,{firstPlayableAudioMs:performance.now()-started,samples:value.samples.length,sampleRate:value.sampleRate});if(value?.type==='failed')finish(3)});child.once('error',()=>finish(5));child.once('exit',()=>finish(6));`],{encoding:'utf8',timeout:20_000,maxBuffer:256*1024,env:environment}),profile=process.env.WAYPOINT_PACKAGE_PERFORMANCE_PROFILE==='hosted-ci'?'hosted-ci':process.env.WAYPOINT_PACKAGE_PERFORMANCE_PROFILE?'invalid':'target';if(profile==='invalid')throw new Error('Unknown package performance profile');const measured=JSON.parse(benchmark.trim())as{firstPlayableAudioMs:number;samples:number;sampleRate:number};console.log(`Packaged Fast Local metric after readiness (${profile}, ${packagedFastLocalLimit(profile)} ms limit): ${JSON.stringify(measured)}`);if(!validPackagedFastLocalMetric(measured,profile))throw new Error(`Packaged Fast Local latency gate failed: ${JSON.stringify(measured)}`)
