...
export function packagedFastLocalLimit(profile:PackagedPerformanceProfile):number{return profile==='target'?1000:3000}
...
function verifyPackagedFastLocal(resources:string,executable:string,archive:string):void{
  ...
  const modelRoot=path.join(resources,'fast-local/kitten'),worker=path.join(archive,'dist-electron/electron/core/fast-local-speech-worker.js'),workerProbe=execFileSync(executable,['-e',`const{fork}=require('node:child_process'),{writeSync}=require('node:fs'),child=fork(${JSON.stringify(worker)},[${JSON.stringify(modelRoot)},'--probe'],{stdio:['ignore','ignore','ignore','ipc'],serialization:'advanced',execArgv:[],env:{ELECTRON_RUN_AS_NODE:'1',PATH:process.env.PATH||'',LANG:process.env.LANG||'en_US.UTF-8'}});let settled=false;const finish=(code)=>{if(settled)return;settled=true;clearTimeout(timer);if(code===0)writeSync(1,'ready');if(child.connected)child.disconnect();if(!child.killed)child.kill('SIGTERM');process.exit(code)},timer=setTimeout(()=>finish(4),15000);child.on('message',value=>finish(value?.type==='probe'&&value.ready===true?0:3));child.once('error',()=>finish(5));child.once('exit',()=>finish(6));`],{encoding:'utf8',timeout:20_000,maxBuffer:256*1024,env:environment});if(workerProbe!=='ready')throw new Error('Packaged Fast Local worker readiness probe failed');const benchmark=execFileSync(executable,['-e',`const{fork}=require('node:child_process'),{writeSync}=require('node:fs'),started=performance.now(),child=fork(${JSON.stringify(worker)},[${JSON.stringify(modelRoot)}],{stdio:['ignore','ignore','ignore','ipc'],serialization:'advanced',execArgv:[],env:{ELECTRON_RUN_AS_NODE:'1',PATH:process.env.PATH||'',LANG:process.env.LANG||'en_US.UTF-8'}});let settled=false;const finish=(code,value)=>{if(settled)return;settled=true;clearTimeout(timer);if(value)writeSync(1,JSON.stringify(value));if(child.connected)child.disconnect();if(!child.killed)child.kill('SIGTERM');process.exit(code)},timer=setTimeout(()=>finish(4),15000);child.on('message',value=>{if(value?.type==='audio'&&value.samples instanceof Float32Array&&value.samples.length&&value.sampleRate===24000)finish(0,{firstPlayableAudioMs:performance.now()-started,samples:value.samples.length,sampleRate:value.sampleRate});else finish(3)});child.once('spawn',()=>child.send({type:'speak',text:${JSON.stringify(packagedFastLocalOpening)}}));child.once('error',()=>finish(5));child.once('exit',()=>finish(6));`],{encoding:'utf8',timeout:20_000,maxBuffer:256*1024,env:environment}),profile=process.env.WAYPOINT_PACKAGE_PERFORMANCE_PROFILE==='hosted-ci'?'hosted-ci':process.env.WAYPOINT_PACKAGE_PERFORMANCE_PROFILE?'invalid':'target';if(profile==='invalid')throw new Error('Unknown package performance profile');const measured=JSON.parse(benchmark.trim())as{firstPlayableAudioMs:number;samples:number;sampleRate:number};console.log(`Packaged Fast Local metric after readiness (${profile}, ${packagedFastLocalLimit(profile)} ms limit): ${JSON.stringify(measured)}`);if(!validPackagedFastLocalMetric(measured,profile))throw new Error(`Packaged Fast Local latency gate failed: ${JSON.stringify(measured)}`)
