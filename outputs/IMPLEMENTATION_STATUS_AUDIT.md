...
| Multi-provider/model routing and multi-agent execution | R5 provides versioned local Codex/Claude routing, bounded one-child lineage, execution receipts/budgets, and local embedding/chunking evaluation | Next safe P6A/P6B plan adds a no-effect paired-run harness and trusted-main-process Tool Gateway simulator. Kimi/DeepSeek/OpenRouter, APIs/costs, browser/network use, peer execution, PRs, deployments, and external accounts remain explicit activation gates |
