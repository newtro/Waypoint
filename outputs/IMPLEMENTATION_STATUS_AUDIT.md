...
| Multi-provider/model routing and multi-agent execution | R5 provides versioned local Codex/Claude routing, bounded one-child lineage, execution receipts/budgets, and local embedding/chunking evaluation. P6B provides the trusted-main tool contract, interactive local terminal/CLI adapter, domain-only AI bridge, UI observability, and native-CLI chat timeline. P6C now provides protected workspace-scoped failure preflight, reasoned retry, supersession, bounded retention, lifecycle and readable receipts | P6A paired provider evaluation and live hosted-provider connection remain gated. AI-origin inherited terminal is deliberately denied until an enforceable no-exfiltration boundary exists. Kimi/DeepSeek/OpenRouter, APIs/costs, browser/network use, peer execution, PRs, deployments, Windows process-tree proof, and external accounts remain explicit activation gates |
