...
- Shared GitHub runners are native package-closure infrastructure, not representative Waypoint user hardware. Keep the default/local target profile at one second, but give the explicitly selected hosted-CI profile a hard three-second ceiling while logging its exact production-path measurement. Run the packaged readiness/model probe first because the app performs that probe on workspace load before voice use; an unprimed diagnostic is cold-start evidence, not the interactive path. Hosted results prove runnable bounded packages, not product-latency acceptance; Windows remains a real-device latency check.
...
- 2026-08-20: Ad-hoc macOS packages have version-specific designated requirements, so replacing a local test build can require removing/re-granting only Waypoint in Screen Recording Settings and relaunching even when the prior toggle remains visible. A prototype persistent self-signed identity was rejected: imported local code-signing keys can give Apple signing tools enough authority to impersonate Waypoint's TCC identity. Stable permission continuity therefore remains gated on an authorized Apple Development or Developer ID identity with controlled key custody; Waypoint does not install a broad local signing trust root.
- 2026-08-21: A reset or fresh ad-hoc install may initially report Screen Recording as denied before macOS has displayed consent. Waypoint must allow the explicit user capture action to invoke the real desktop-capture request, then fail closed with relaunch guidance if consent is not yet active; it must not infer a stale identity and block the request before the OS can ask.
- 2026-08-20: Provider text is canonicalized as message sections, not a delimiter-free token stream. Codex app-server item IDs are retained in durable event metadata; legacy Codex/Grok runs recover only boundaries proven by intervening durable non-text events. Schema 48 rewrites an existing answer only when it exactly matches the old mechanical join, preserving user edits and special answer overrides while keeping the search index consistent.

# 2026-08-21 — Truthful model thinking and self-contained device invitations

- Thinking level is a per-workspace, device-local model preference. The selector is driven by the exact installed Codex inventory or reviewed provider/model capability metadata; **Model default** is the only choice when the active model cannot be identified. Unsupported saved combinations remain visible but cannot be dispatched, and changing models resets an incompatible level.
- Codex uses app-server `turn/start.effort`; Claude uses Agent SDK `effort`; Grok Build uses its documented `--reasoning-effort`; OpenRouter uses the normalized `reasoning.effort` request field. Explicit effort is recorded as a bounded policy event without exposing prompts or secrets.
- In desktop-host sync mode, **Invite device** starts the host only after explicit confirmation when necessary, bounds invitation registration to twelve seconds, and presents the one-use token in Settings until dismissed. Clipboard access is a convenience, never the sole delivery or success state.
- Desktop-host HTTPS continues to trust only the exact protected leaf certificate in the host descriptor. Electron's embedded OpenSSL requires partial-chain trust to accept that directly pinned self-signed leaf; this is enabled only on the pinned request path and does not add a system or public CA.
- Settings theme completeness includes helper text, native form controls, disabled states, invitation cards, model controls, and responsive dark/light presentation. No-warning verification remains mandatory.
