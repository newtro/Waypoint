...
The chat header and execution timeline show streaming, completion, cancellation, failure, provider, model, and redacted tool events when the underlying CLI supplies them. Provider-authored assistant sections are persisted with their message boundaries so progress commentary and the final answer do not collapse into one text blob after restart. The structured tool/reasoning trace is durably retained and collapsed separately above the final answer. Useful partial assistant text remains visible if a run is canceled or fails.
