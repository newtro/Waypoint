...
contextBridge.exposeInMainWorld('waypoint', {
  ...
  createMeeting:(workspaceId:string,title:string,consentAcknowledged:boolean)=>ipcRenderer.invoke('waypoint:create-meeting',{workspaceId,title,consentAcknowledged}),
  finalizeMeeting:(workspaceId:string,meetingId:string,mediaType:string,audio:Uint8Array)=>ipcRenderer.invoke('waypoint:finalize-meeting',{workspaceId,meetingId,mediaType,audio}),
  failMeeting:(workspaceId:string,meetingId:string,failureCode:string)=>ipcRenderer.invoke('waypoint:fail-meeting',{workspaceId,meetingId,failureCode}),
  listMeetings:(workspaceId:string)=>ipcRenderer.invoke('waypoint:list-meetings',{workspaceId}),
  updateMeetingTranscript:(workspaceId:string,meetingId:string,transcript:string,reviewed:boolean)=>ipcRenderer.invoke('waypoint:update-meeting-transcript',{workspaceId,meetingId,transcript,reviewed}),
  saveMeetingMemory:(workspaceId:string,meetingId:string)=>ipcRenderer.invoke('waypoint:save-meeting-memory',{workspaceId,meetingId}),
  deleteMeeting:(workspaceId:string,meetingId:string)=>ipcRenderer.invoke('waypoint:delete-meeting',{workspaceId,meetingId}),
  readMeetingAudio:(workspaceId:string,meetingId:string)=>ipcRenderer.invoke('waypoint:read-meeting-audio',{workspaceId,meetingId}),
  exportMeetingAudio:(workspaceId:string,meetingId:string)=>ipcRenderer.invoke('waypoint:export-meeting-audio',{workspaceId,meetingId}),
  ...
  scanMemorySuggestions:(workspaceId:string,chatId?:string)=>ipcRenderer.invoke('waypoint:scan-memory-suggestions',{workspaceId,chatId}),
  ...
  dryRunRuleSuggestion:(workspaceId:string,suggestionId:string)=>ipcRenderer.invoke('waypoint:dry-run-rule-suggestion',{workspaceId,suggestionId}),
  resolveRuleSuggestion:(workspaceId:string,suggestionId:string,action:'approve'|'reject')=>ipcRenderer.invoke('waypoint:resolve-rule-suggestion',{workspaceId,suggestionId,action}),
  listLearnedRules:(workspaceId:string)=>ipcRenderer.invoke('waypoint:list-learned-rules',{workspaceId}),
  setLearnedRuleEnabled:(workspaceId:string,ruleId:string,enabled:boolean)=>ipcRenderer.invoke('waypoint:set-learned-rule-enabled',{workspaceId,ruleId,enabled}),
