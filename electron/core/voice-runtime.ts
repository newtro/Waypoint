...
export class NativeSpeechAdapter {
  private child?: ChildProcess; private sequence = 0;private queue:string[]=[];
  ...
  speak(text: string, onDone: (result: 'completed' | 'canceled' | 'failed') => void) { if (!this.available()) throw new Error('voice_tts_unavailable'); if (!text.trim() || text.length > 200_000) throw new Error('voice_tts_text_invalid'); this.stop();const sequence=++this.sequence;this.queue=segmentSpeechText(text);let finished=false;const finish=(result:'completed'|'canceled'|'failed')=>{if(finished||sequence!==this.sequence)return;finished=true;this.queue=[];this.child=undefined;onDone(result)},next=()=>{if(finished||sequence!==this.sequence)return;const segment=this.queue.shift();if(!segment){finish('completed');return}const child=this.launch('/usr/bin/say',['--',segment],{stdio:'ignore'});this.child=child;child.once('error',()=>finish('failed'));child.once('exit',(code,signal)=>{if(finished||sequence!==this.sequence)return;this.child=undefined;if(signal||code!==0){finish(signal?'canceled':'failed');return}next()})};next();return()=>this.stop() }
  stop() { this.sequence++;this.queue=[]; const child = this.child; this.child = undefined; if (child && !child.killed) child.kill('SIGTERM'); }
}

export function segmentSpeechText(text:string,maxCharacters=260):string[]{const normalized=text.replace(/\s+/g,' ').trim();if(!normalized)return[];const sentences=normalized.match(/[^.!?]+(?:[.!?]+["')\]]*|$)/g)??[normalized],result:string[]=[];let current='';for(const raw of sentences){let sentence=raw.trim();while(sentence.length>maxCharacters){const breakAt=Math.max(sentence.lastIndexOf(' ',maxCharacters),Math.floor(maxCharacters*.65)),part=sentence.slice(0,breakAt).trim();if(current)result.push(current);result.push(part);current='';sentence=sentence.slice(breakAt).trim()}if(!sentence)continue;if(current&&current.length+1+sentence.length>maxCharacters){result.push(current);current=sentence}else current=current?`${current} ${sentence}`:sentence}if(current)result.push(current);return result}
