...
export class WorkspaceSyncJournal{
  constructor(private readonly db:DatabaseSync){WorkspaceSyncJournal.install(db)}
  static install(db:DatabaseSync):void{db.exec(`
  ...
  enqueue(workspaceId:string,objectId:string,objectKind:string,operation:SyncOperation,payload:unknown,ownedIds:string[]=[]):LocalMutation{
    ...
    const sequence=(prior?.sequence??0)+1,head=this.db.prepare('SELECT clock_json clock FROM sync_heads WHERE workspace_id=? AND object_id=?').get(workspaceId,objectId) as {clock:string}|undefined,clock={...(head?JSON.parse(head.clock) as CausalClock:{}),[state.deviceId]:sequence},id=randomUUID(),createdAt=now()
    ...
    if(operation==='delete'){
      this.db.prepare('DELETE FROM sync_attachment_outbound WHERE workspace_id=? AND object_id=?').run(workspaceId,objectId)
  ...
  pending(workspaceId:string):LocalMutation[]{const pending=(this.db.prepare('SELECT * FROM sync_mutations WHERE workspace_id=? ORDER BY device_id,sequence').all(workspaceId) as Array<Record<string,unknown>>).map((row)=>({id:String(row.id),workspaceId:String(row.workspace_id),objectId:String(row.object_id),objectKind:String(row.object_kind),operation:row.operation as SyncOperation,deviceId:String(row.device_id),sequence:Number(row.sequence),clock:JSON.parse(String(row.clock_json)),payload:JSON.parse(String(row.payload_json)),createdAt:String(row.created_at)}));const ids=new Set(pending.map((item)=>item.id)),resumable=(this.db.prepare('SELECT mutation_json mutation FROM sync_attachment_outbound o WHERE workspace_id=? AND EXISTS(SELECT 1 FROM sync_attachment_missing_requests r WHERE r.workspace_id=o.workspace_id AND r.transfer_id=o.transfer_id)').all(workspaceId) as Array<{mutation:string}>).map((row)=>JSON.parse(row.mutation) as LocalMutation).filter((item)=>!ids.has(item.id));return[...pending,...resumable]}
  markRelayed(workspaceId:string,mutationId:string):void{const row=this.db.prepare('SELECT * FROM sync_mutations WHERE id=? AND workspace_id=?').get(mutationId,workspaceId) as Record<string,unknown>|undefined;if(row){if(row.object_kind==='attachment'&&row.operation==='upsert'){const mutation:LocalMutation={id:String(row.id),workspaceId:String(row.workspace_id),objectId:String(row.object_id),objectKind:String(row.object_kind),operation:row.operation as SyncOperation,deviceId:String(row.device_id),sequence:Number(row.sequence),clock:JSON.parse(String(row.clock_json)),payload:JSON.parse(String(row.payload_json)),createdAt:String(row.created_at)};this.db.prepare('INSERT OR REPLACE INTO sync_attachment_outbound VALUES (?,?,?,?,?)').run(mutation.id,workspaceId,mutation.objectId,JSON.stringify(mutation),now())}this.db.prepare('DELETE FROM sync_mutations WHERE id=? AND workspace_id=?').run(mutationId,workspaceId);this.db.prepare('DELETE FROM sync_snapshot_targets WHERE mutation_id=? AND workspace_id=?').run(mutationId,workspaceId);return}if(!this.db.prepare('SELECT 1 FROM sync_attachment_outbound WHERE transfer_id=? AND workspace_id=?').get(mutationId,workspaceId))throw new Error('Pending sync mutation not found')}
  targetMutation(workspaceId:string,mutationId:string,recipientDeviceId:string):void{this.db.prepare('INSERT OR REPLACE INTO sync_snapshot_targets VALUES (?,?,?)').run(mutationId,workspaceId,recipientDeviceId)}
  mutationTarget(workspaceId:string,mutationId:string):string|undefined{return (this.db.prepare('SELECT recipient_device_id recipient FROM sync_snapshot_targets WHERE workspace_id=? AND mutation_id=?').get(workspaceId,mutationId) as {recipient:string}|undefined)?.recipient}
  recordSnapshotRequest(workspaceId:string,requestId:string,ownerDeviceId:string):void{const cutoff=new Date(Date.now()-86_400_000).toISOString();this.db.prepare('DELETE FROM sync_snapshot_requests WHERE created_at<?').run(cutoff);const count=Number((this.db.prepare('SELECT count(*) count FROM sync_snapshot_requests WHERE workspace_id=?').get(workspaceId) as {count:number}).count);if(count>=100)throw new Error('Outstanding snapshot request limit reached');this.db.prepare('INSERT INTO sync_snapshot_requests VALUES (?,?,?,?,NULL)').run(requestId,workspaceId,ownerDeviceId,now())}
  consumeSnapshotResponse(workspaceId:string,requestId:string,ownerDeviceId:string):boolean{const cutoff=new Date(Date.now()-86_400_000).toISOString();return Boolean(this.db.prepare('SELECT 1 FROM sync_snapshot_requests WHERE request_id=? AND workspace_id=? AND owner_device_id=? AND created_at>=? AND consumed_at IS NULL').get(requestId,workspaceId,ownerDeviceId,cutoff))}
  completeSnapshotResponse(workspaceId:string,requestId:string,ownerDeviceId:string):void{this.db.prepare('UPDATE sync_snapshot_requests SET consumed_at=? WHERE request_id=? AND workspace_id=? AND owner_device_id=? AND consumed_at IS NULL').run(now(),requestId,workspaceId,ownerDeviceId)}
  removeSnapshotRequest(workspaceId:string,requestId:string):void{this.db.prepare('DELETE FROM sync_snapshot_requests WHERE workspace_id=? AND request_id=? AND consumed_at IS NULL').run(workspaceId,requestId)}
  hasAppliedChange(changeId:string):boolean{return Boolean(this.db.prepare('SELECT 1 FROM sync_applied_changes WHERE change_id=?').get(changeId))}
  stageAttachment(changeId:string,workspaceId:string,payload:Record<string,unknown>,chunkCount:number):void{if(!Number.isSafeInteger(chunkCount)||chunkCount<1||chunkCount>7||!Number.isSafeInteger(payload.bytes)||Number(payload.bytes)<1||Number(payload.bytes)>25*1024*1024)throw new Error('Invalid attachment transfer bounds');this.db.prepare('INSERT INTO sync_attachment_manifests VALUES (?,?,?,?,?,?,?,?,?,?) ON CONFLICT(transfer_id) DO NOTHING').run(workspaceId,changeId,String(payload.id),String(payload.ownerId),String(payload.name),String(payload.mediaType),String(payload.sha256),Number(payload.bytes),chunkCount,String(payload.createdAt))}
  acceptAttachmentChunk(workspaceId:string,transferId:string,index:number,total:number,plaintext:Uint8Array):{complete:boolean;manifest?:Record<string,unknown>;bytes?:Uint8Array}{const manifest=this.db.prepare('SELECT * FROM sync_attachment_manifests WHERE workspace_id=? AND transfer_id=?').get(workspaceId,transferId) as Record<string,unknown>|undefined;if(!manifest||Number(manifest.chunk_count)!==total||!Number.isSafeInteger(index)||index<0||index>=total||plaintext.byteLength>4*1024*1024)throw new Error('Attachment chunk has no matching manifest');const stored=Number((this.db.prepare('SELECT COALESCE(SUM(length(plaintext)),0) bytes FROM sync_attachment_chunks WHERE workspace_id=? AND transfer_id=?').get(workspaceId,transferId) as {bytes:number}).bytes);if(stored+plaintext.byteLength>Number(manifest.total_bytes))throw new Error('Attachment chunks exceed manifest size');this.db.prepare('INSERT OR IGNORE INTO sync_attachment_chunks VALUES (?,?,?,?,?)').run(workspaceId,transferId,index,total,Buffer.from(plaintext));const rows=this.db.prepare('SELECT chunk_index,plaintext FROM sync_attachment_chunks WHERE workspace_id=? AND transfer_id=? ORDER BY chunk_index').all(workspaceId,transferId) as Array<{chunk_index:number;plaintext:Uint8Array}>;if(rows.length!==total)return{complete:false};if(rows.some((row,expected)=>row.chunk_index!==expected))throw new Error('Attachment chunk sequence is incomplete');const buffers=rows.map((row)=>Buffer.from(row.plaintext)),bytes=Buffer.concat(buffers);if(bytes.byteLength!==Number(manifest.total_bytes)||createHash('sha256').update(bytes).digest('hex')!==String(manifest.sha256))throw new Error('Attachment transfer integrity mismatch');return{complete:true,manifest,bytes:new Uint8Array(bytes)}}
  finishAttachment(transferId:string):void{this.db.prepare('DELETE FROM sync_attachment_chunks WHERE transfer_id=?').run(transferId);this.db.prepare('DELETE FROM sync_attachment_manifests WHERE transfer_id=?').run(transferId)}
  missingAttachmentChunks(workspaceId:string,transferId:string,total:number):number[]{const received=new Set((this.db.prepare('SELECT chunk_index FROM sync_attachment_chunks WHERE workspace_id=? AND transfer_id=?').all(workspaceId,transferId) as Array<{chunk_index:number}>).map((row)=>row.chunk_index));return Array.from({length:total},(_,index)=>index).filter((index)=>!received.has(index))}
  recordAttachmentMissing(workspaceId:string,transferId:string,peerDeviceId:string,indices:number[]):void{const cutoff=new Date(Date.now()-R0_PROTOCOL_CONTRACT.retention.relayEnvelopeMaximumDays*86_400_000).toISOString();this.db.prepare('DELETE FROM sync_attachment_outbound WHERE retained_at<?').run(cutoff);this.db.prepare('DELETE FROM sync_attachment_missing_requests WHERE updated_at<?').run(cutoff);if(indices.length>7||new Set(indices).size!==indices.length||indices.some((index)=>!Number.isSafeInteger(index)||index<0||index>6)||!(this.db.prepare('SELECT 1 FROM sync_attachment_outbound WHERE workspace_id=? AND transfer_id=?').get(workspaceId,transferId)||this.db.prepare("SELECT 1 FROM sync_mutations WHERE workspace_id=? AND id=? AND object_kind='attachment' AND operation='upsert'").get(workspaceId,transferId)))throw new Error('Attachment missing-index request is invalid');this.db.prepare('INSERT INTO sync_attachment_missing_requests VALUES (?,?,?,?,?) ON CONFLICT(transfer_id,peer_device_id) DO UPDATE SET indices_json=excluded.indices_json,updated_at=excluded.updated_at').run(workspaceId,transferId,peerDeviceId,JSON.stringify(indices),now())}
  requestedAttachmentChunks(workspaceId:string,transferId:string,peerDeviceId:string):number[]|undefined{const row=this.db.prepare('SELECT indices_json indices FROM sync_attachment_missing_requests WHERE workspace_id=? AND transfer_id=? AND peer_device_id=?').get(workspaceId,transferId,peerDeviceId) as {indices:string}|undefined;return row?JSON.parse(row.indices) as number[]:undefined}
  clearAttachmentRequest(transferId:string,peerDeviceId:string):void{this.db.prepare('DELETE FROM sync_attachment_missing_requests WHERE transfer_id=? AND peer_device_id=?').run(transferId,peerDeviceId)}
  quarantine(workspaceId:string,envelopeId:string,senderDeviceId:string,reasonCode:string):void{this.db.prepare('INSERT OR IGNORE INTO sync_quarantine VALUES (?,?,?,?,?)').run(envelopeId,workspaceId,senderDeviceId,reasonCode.slice(0,64),now())}
  consumeControlRequest(workspaceId:string,requestId:string,senderDeviceId:string):boolean{this.db.prepare('DELETE FROM sync_control_requests WHERE consumed_at<?').run(new Date(Date.now()-86_400_000).toISOString());const count=Number((this.db.prepare('SELECT count(*) count FROM sync_control_requests WHERE workspace_id=?').get(workspaceId) as {count:number}).count);if(count>=1000)throw new Error('Snapshot request dedupe limit reached');return Boolean(this.db.prepare('INSERT OR IGNORE INTO sync_control_requests VALUES (?,?,?,?)').run(requestId,workspaceId,senderDeviceId,now()).changes)}
  cascadeTombstone(workspaceId:string,objectId:string,changeId:string):void{this.db.prepare('DELETE FROM sync_conflicts WHERE workspace_id=? AND object_id=?').run(workspaceId,objectId);this.db.prepare('DELETE FROM sync_heads WHERE workspace_id=? AND object_id=?').run(workspaceId,objectId);this.db.prepare('INSERT INTO sync_tombstones VALUES (?,?,?,?,?) ON CONFLICT(workspace_id,object_id) DO UPDATE SET change_id=excluded.change_id,retain_until=excluded.retain_until,created_at=excluded.created_at').run(workspaceId,objectId,changeId,new Date(Date.now()+RETENTION_MS).toISOString(),now())}
