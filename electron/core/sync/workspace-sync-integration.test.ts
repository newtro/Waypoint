...
describe('WorkspaceStore Phase 3 journal integration',()=>{
  ...
  it('rolls back an entire replacement snapshot when attachment staging fails',()=>{const{root,database,store,workspace}=setup(),chat=store.captureChat(workspace.id,'Route','body'),source=path.join(root,'source.md');writeFileSync(source,'payload');store.addAttachment(workspace.id,chat,'source.md','text/markdown',source);const admin=new DatabaseSync(database),row=admin.prepare('SELECT relative_path path FROM attachments WHERE workspace_id=?').get(workspace.id) as {path:string};admin.close();rmSync(path.join(path.dirname(database),'attachments',row.path));const before=store.pendingSyncChanges(workspace.id).map((item)=>item.id);expect(()=>store.queueReplacementSnapshot(workspace.id,'snapshot-request-atomic','requesting-device-01')).toThrow();expect(store.pendingSyncChanges(workspace.id).map((item)=>item.id)).toEqual(before);store.close()})
  it('rejects cross-workspace object substitution before canonical mutation',()=>{const{store,workspace}=setup(),other=store.createWorkspace('Other','/tmp/waypoint-other'),foreign=store.createDocument(other.id,'Foreign','secret');const attack=inbound({workspaceId:workspace.id,objectId:foreign.id,id:'cross-workspace-change',deviceId:'peer',sequence:1,clock:{peer:1},payload:{id:foreign.id,title:'Changed',revisionId:'foreign-revision',body:'stolen'}});expect(()=>store.applyInboundSyncChange(attack)).toThrow('another workspace');expect(store.listDocuments(other.id)[0]).toMatchObject({title:'Foreign',body:'secret'});store.close()})
  it('causally tombstones cascade children and rejects oversized attachment manifests',()=>{const{store,workspace}=setup(),chatId='remote-chat-0001',messageId='remote-message-01',createChat=inbound({workspaceId:workspace.id,objectId:chatId,objectKind:'chat',id:'create-chat-change',deviceId:'peer',sequence:1,clock:{peer:1},payload:{id:chatId,title:'Remote'}}),createMessage=inbound({workspaceId:workspace.id,objectId:messageId,objectKind:'message',id:'create-message-change',deviceId:'peer',sequence:2,clock:{peer:2},payload:{id:messageId,chatId,role:'user',body:'hello'}});store.applyInboundSyncChange(createChat);store.applyInboundSyncChange(createMessage);const deletion=inbound({workspaceId:workspace.id,objectId:chatId,objectKind:'chat',operation:'delete',id:'delete-chat-change',deviceId:'peer',sequence:3,clock:{peer:3},payload:{id:chatId,cascade:true,cascadeIds:[chatId,messageId]}});store.applyInboundSyncChange(deletion);expect(store.applyInboundSyncChange({...createMessage,id:'stale-child-change',envelopeId:'stale-child-envelope',sequence:4,clock:{peer:2}})).toBe('ignored');const oversized=inbound({workspaceId:workspace.id,objectId:'remote-attachment',objectKind:'attachment',id:'oversized-attachment',deviceId:'peer',sequence:5,clock:{peer:5},payload:{id:'remote-attachment',ownerId:chatId,name:'large.pdf',mediaType:'application/pdf',sha256:'a'.repeat(64),bytes:26*1024*1024}});expect(()=>store.applyInboundSyncChange(oversized,7)).toThrow('limits');store.close()})
})
