...
describe('ordered schema migration', () => {
  it('snapshots known old data and applies each migration once', () => {
    ...
    runMigrations(db,schemaVersion(db),[{version:33,apply:(database)=>database.exec('CREATE TABLE screen_captures(id TEXT PRIMARY KEY)')}])
    runMigrations(db,schemaVersion(db),[{version:34,apply:(database)=>database.exec('CREATE TABLE quick_capture_settings(id TEXT PRIMARY KEY)')}])
    runMigrations(db,schemaVersion(db),[{version:35,apply:(database)=>database.exec('CREATE TABLE automation_proposals(id TEXT PRIMARY KEY)')}])
    runMigrations(db,schemaVersion(db),[{version:36,apply:(database)=>database.exec('CREATE TABLE automation_runs(id TEXT PRIMARY KEY)')}])
    runMigrations(db,schemaVersion(db),[{version:37,apply:(database)=>database.exec('CREATE TABLE automation_provisioning_events(id TEXT PRIMARY KEY)')}])
    runMigrations(db,schemaVersion(db),[{version:38,apply:(database)=>database.exec('CREATE TABLE screen_capture_workflow_repair(id TEXT PRIMARY KEY)')}])
    runMigrations(db,schemaVersion(db),[{version:39,apply:(database)=>database.exec('CREATE TABLE provider_attachment_routes(id TEXT PRIMARY KEY)')}])
