...
export type ExecutionEvent = {
  type: "text" | "tool" | "agent" | "diagnostic";
  text?: string;
  name?: string;
  rawType?: string;
...
export const CONSERVATIVE_PROFILE: SecurityProfile = {
  id: "workspace-conservative-v1",
  name: "Workspace — conservative",
  roots: [],
...
export function validateRequest(request: RunRequest): void {
  ...
  if (
    !request.profile.roots.length ||
    !request.profile.roots.some((root) => isWithin(request.workspaceRoot, root))
  )
    throw new Error("Workspace root is outside the security profile");
  if ((request.depth ?? 0) > 1)
    throw new Error("Agent lineage depth exceeds the Phase 2 limit");
  if (request.profile.maxConcurrency !== 1)
...
export function adapterInput(
  ...
  prompt: string,
  images: CliImageInput[] = [],
  imageDataBase64: readonly string[] = [],
): string {
  if (cli !== "claude" || !images.length) return prompt;
  if (imageDataBase64.length !== images.length)
    throw new Error("Validated Claude image data is incomplete");
...
export function parseEvent(cli: CliName, line: string): ExecutionEvent {
  ...
  try {
    ...
    return { type: "diagnostic", text: "CLI emitted an unparseable event" };
  }
  if (cli === "codex") {
    ...
    if (item?.type === "command_execution") {
      const phase = String(value.type ?? "").includes("started")
          ? "started"
          : String(value.type ?? "").includes("completed")
            ? "completed"
            : "progress",
        exit =
        ...
        output =
          typeof item.aggregated_output === "string"
            ? redactToolText(item.aggregated_output).trim().slice(0, 1000)
            : undefined;
      return {
        type: "tool",
        name: `Command ${phase}${exit}`,
        text: output,
        rawType: String(value.type ?? ""),
      };
    }
  } else {
    const streamEvent = value.event as Record<string, unknown> | undefined,
      delta = streamEvent?.delta as Record<string, unknown> | undefined;
    if (
      ...
      return {
        type: "text",
        text: delta.text,
        rawType: "stream_event.content_block_delta",
      };
    if (
      value.type === "system" &&
      value.subtype === "init" &&
    ...
    if (Array.isArray(content)) {
      ...
      if (result)
        return {
          type: "tool",
          name: `Tool ${result.is_error === true ? "failed" : "completed"}`,
          text:
            typeof result.content === "string"
              ? redactToolText(result.content).slice(0, 1000)
              : undefined,
          rawType: String(value.type ?? ""),
        };
      ...
      if (tool)
        return {
          type: "tool",
          name: `${String(tool.name ?? "tool")} started`,
          rawType: String(value.type ?? ""),
        };
      const text = content
        .filter(
        ...
        .join("");
      if (text)
        return { type: "text", text, rawType: String(value.type ?? "") };
    }
  }
...
export class CliWorkbench {
  ...
  async start(
    ...
    if (!executable)
      throw new Error(`${request.cli} CLI was not found on PATH`);
    ...
    for (const image of images) {
      ...
      const validated = validateAttachment(image.name, image.mediaType, bytes);
      imageDimensions(image.mediaType, bytes);
      if (validated.sha256 !== image.sha256)
        throw new Error("Attachment image integrity check failed");
      imageBytes.push(bytes);
      if (request.cli === "claude")
        imageDataBase64.push(bytes.toString("base64"));
    }
    let snapshotRoot: string | undefined,
      deliveredImages = images;
    ...
    try {
      if (request.cli === "codex" && images.length) {
        ...
        deliveredImages = images.map((image, index) => {
          const snapshotPath = path.join(
            snapshotRoot!,
            `${index}${path.extname(image.name).toLowerCase()}`,
          );
          writeFileSync(snapshotPath, imageBytes[index], {
            flag: "wx",
            mode: 0o600,
          });
          return { ...image, path: snapshotPath };
        });
      }
      const args = adapterArgs(
        ...
        environment = cliExecutionEnvironment(
          executable,
          process.env,
          this.platform,
        ),
        invocation = await this.invocationResolver(
          request.cli,
          executable,
          args,
      ...
      child.stdin.end(
        adapterInput(request.cli, request.prompt, images, imageDataBase64),
      );
      child.stdout.on("data", (chunk: string) => {
        ...
        for (const line of lines)
          if (line.trim()) onEvent(parseEvent(request.cli, line));
      });
      ...
      const completion = new Promise<{
        ...
        child.once("close", (code) => {
          ...
          this.active.delete(runId);
          cleanupSnapshots();
          if (buffer.trim()) onEvent(parseEvent(request.cli, buffer));
          ...
          resolve({
            ...
            error:
              status === "failed"
                ? stderr.trim() || `CLI exited with code ${code}`
                : undefined,
          });
        });
      });
