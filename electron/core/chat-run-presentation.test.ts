...
} from "../../src/chat-run-presentation.js";
...
describe("chat execution presentation", () => {
  ...
  it("associates execution history with its source turn so the assistant reply can render below it", () => {
    ...
    expect(runsForSourceMessage(runs, "user-1").map((run) => run.id)).toEqual([
      "current",
    ]);
  });

  it.each(["completed", "failed", "canceled", "timed_out"])(
    "clears a response notice on terminal %s",
    (status) => {
      expect(
        responseNoticeAfterRuns("codex is responding…", [
          { id: "run", status },
        ]),
      ).toBe("");
    },
  ...
  it("retains a response notice only while the exact refreshed chat still has active work", () => {
    expect(
      responseNoticeAfterRuns("claude is responding…", [
        { id: "run", status: "running" },
      ]),
    ).toBe("claude is responding…");
    expect(
      responseNoticeAfterRuns("Preferences saved.", [
  ...
  it("renders source-associated history before the following reply and keeps header actions grouped accessibly", () => {
    const source = readFileSync(
      ...
      "utf8",
    ).replace(/\s+/g," "),compact=source.replace(/\s+/g,'').replaceAll('"',"'");
    const styles = readFileSync(
      new URL("../../src/chat-header-actions.css", import.meta.url),
      "utf8",
    );
