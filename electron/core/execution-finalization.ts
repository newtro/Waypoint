...
export async function finalizeExecution(
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    if (!store.executionExists(input.workspaceId, input.runId))
      return "owner-deleted";
    try {
      const storedEvents = store
        .listExecutions(input.workspaceId, input.chatId)
        .find((run) => run.id === input.runId)?.events;
      const durableEvents = Array.isArray(storedEvents)
        ? storedEvents.filter(
            (event): event is Record<string, unknown> =>
              Boolean(event) && typeof event === "object",
          )
        : [];
      const events: Array<Record<string, unknown>> = input.fallbackEvents
        ?.length
        ? input.fallbackEvents
        : durableEvents;
      const answer =
        input.answerOverride ?? canonicalExecutionText(input.cli, events);
