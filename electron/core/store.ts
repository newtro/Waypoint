...
const TOOL_IDENTITIES=['workspace.list_files','workspace.read_file','workspace.search','workspace.write_file','terminal.run','local_cli.run','agent_browser.run','waypoint.command'] as const;
...
export class WorkspaceStore {
  ...
  restoreWorkspace(archive: ExportArchive, newName: string, newLocalPath: string): WorkspaceSummary {
    ...
    try {
      ...
      this.ensureAutonomousDeveloperProfile(workspace.id, workspace.localPath);
        ...
        for(const value of archive.objects.tool_gateway_settings??[]){const row=value as Record<string,unknown>,patterns=JSON.parse(String(row.deny_patterns_json)),mode=row.browser_profile_mode==='existing'?'existing':'isolated',profile=typeof row.browser_profile_name==='string'&&/^[ A-Za-z0-9._-]{1,100}$/.test(row.browser_profile_name)?row.browser_profile_name:'Default',domains=row.browser_allowed_domains_json?JSON.parse(String(row.browser_allowed_domains_json)):[];if(!Array.isArray(patterns)||patterns.length>100||patterns.some((item)=>typeof item!=='string'||item.length>300)||!Array.isArray(domains)||domains.length>30||domains.some((item)=>typeof item!=='string'||!isPublicBrowserDomain(item)))throw new Error('Tool gateway settings archive is invalid');for(const pattern of patterns)try{new RegExp(pattern,'i')}catch{throw new Error('Tool gateway settings archive is invalid')}this.db.prepare('INSERT INTO tool_gateway_settings VALUES (?,?,?,?,?,?,?,?,?)').run(workspace.id,0,JSON.stringify(patterns),Number(row.suppress_commit)===1?1:0,Number(row.suppress_push)===1?1:0,String(row.updated_at),mode,profile,JSON.stringify(domains))}
  ...
  toolGatewaySettings(workspaceId:string):{stopped:boolean;denyPatterns:string[];suppressCommit:boolean;suppressPush:boolean;browserProfileMode:'existing'|'isolated';browserProfileName:string;browserAllowedDomains:string[];updatedAt:string}{
    if(!this.db.prepare('SELECT 1 FROM workspaces WHERE id=?').get(workspaceId))throw new Error('Workspace not found')
    const row=this.db.prepare('SELECT stopped,deny_patterns_json denyPatterns,suppress_commit suppressCommit,suppress_push suppressPush,browser_profile_mode browserProfileMode,browser_profile_name browserProfileName,browser_allowed_domains_json browserAllowedDomains,updated_at updatedAt FROM tool_gateway_settings WHERE workspace_id=?').get(workspaceId) as Record<string,unknown>|undefined
    return row?{stopped:Boolean(row.stopped),denyPatterns:JSON.parse(String(row.denyPatterns)) as string[],suppressCommit:Boolean(row.suppressCommit),suppressPush:Boolean(row.suppressPush),browserProfileMode:String(row.browserProfileMode)==='existing'?'existing':'isolated',browserProfileName:String(row.browserProfileName),browserAllowedDomains:JSON.parse(String(row.browserAllowedDomains)) as string[],updatedAt:String(row.updatedAt)}:{stopped:false,denyPatterns:[],suppressCommit:false,suppressPush:false,browserProfileMode:'isolated',browserProfileName:'Default',browserAllowedDomains:[],updatedAt:now()}
  }
  setToolGatewaySettings(workspaceId:string,input:{stopped:boolean;denyPatterns:string[];suppressCommit:boolean;suppressPush:boolean;browserProfileMode?:'existing'|'isolated';browserProfileName?:string;browserAllowedDomains?:string[]}){
    const browserProfileMode=input.browserProfileMode??'isolated',browserProfileName=input.browserProfileName??'Default',browserAllowedDomains=input.browserAllowedDomains??[];if(!this.db.prepare('SELECT 1 FROM workspaces WHERE id=?').get(workspaceId)||!['existing','isolated'].includes(browserProfileMode)||!/^[ A-Za-z0-9._-]{1,100}$/.test(browserProfileName)||browserAllowedDomains.length>30||browserAllowedDomains.some((item)=>!isPublicBrowserDomain(item)))throw new Error('Tool gateway settings are invalid');const timestamp=now(),value={...input,browserProfileMode,browserProfileName,browserAllowedDomains};this.db.prepare('INSERT INTO tool_gateway_settings VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(workspace_id) DO UPDATE SET stopped=excluded.stopped,deny_patterns_json=excluded.deny_patterns_json,suppress_commit=excluded.suppress_commit,suppress_push=excluded.suppress_push,updated_at=excluded.updated_at,browser_profile_mode=excluded.browser_profile_mode,browser_profile_name=excluded.browser_profile_name,browser_allowed_domains_json=excluded.browser_allowed_domains_json').run(workspaceId,input.stopped?1:0,JSON.stringify(input.denyPatterns),input.suppressCommit?1:0,input.suppressPush?1:0,timestamp,browserProfileMode,browserProfileName,JSON.stringify(browserAllowedDomains));this.activity(workspaceId,'ai','tool.policy.updated',workspaceId,'workspace',{stopped:input.stopped,denyCount:input.denyPatterns.length,suppressCommit:input.suppressCommit,suppressPush:input.suppressPush,browserProfileMode,browserDomainCount:browserAllowedDomains.length});return{...value,updatedAt:timestamp}
  }
