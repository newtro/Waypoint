...
export class WorkspaceStore {
  ...
  private materializeInboundDelete(workspaceId: string, objectId: string, kind: string, removeFiles: string[]): void {
    ...
    const selected = kind === 'document' ? 'documents' : kind === 'chat' ? 'chats' : kind === 'memory' ? 'memories' : kind === 'relationship' ? 'relationships' : kind === 'attachment' ? 'attachments' : kind==='tool_failure'?'tool_failure_knowledge':undefined,
      tables = kind === 'any' ? ['documents', 'chats', 'memories', 'relationships', 'attachments','tool_failure_knowledge'] : selected ? [selected] : [];
  ...
  private assertInboundIdentityAvailable(workspaceId: string, id: string): void {
    for (const table of ['documents', 'chats', 'memories', 'relationships', 'attachments','tool_failure_knowledge']) {
