...
import { captureDigest, defaultCaptureShortcut, validateCaptureLayers, validateCaptureSettings, type CaptureLayer, type CaptureMode, type CaptureSettings } from './manual-screen-capture.js';
...
export class WorkspaceStore {
  ...
  restoreWorkspace(archive: ExportArchive, newName: string, newLocalPath: string): WorkspaceSummary {
    ...
    try {
      ...
      this.ensureAutonomousDeveloperProfile(workspace.id, workspace.localPath);
        ...
        const screenSettings=archive.objects.screen_capture_settings??[];if(screenSettings.length>1)throw new Error('Screen capture settings archive is invalid');if(screenSettings.length){const row=screenSettings[0] as Record<string,unknown>,settings=validateCaptureSettings({mode:String(row.mode) as CaptureMode,shortcut:String(row.shortcut),retentionDays:Number(row.retention_days) as 7|30|90,maxCaptures:Number(row.max_captures)});this.db.prepare('INSERT INTO screen_capture_settings VALUES (?,?,?,?,?,?)').run(workspace.id,settings.mode,settings.shortcut,settings.retentionDays,settings.maxCaptures,String(row.updated_at))}
  ...
  screenCaptureSettings(workspaceId: string): CaptureSettings {
    if(!this.db.prepare('SELECT 1 FROM workspaces WHERE id=?').get(workspaceId))throw new Error('Workspace not found')
    const row = this.db.prepare('SELECT mode,shortcut,retention_days retentionDays,max_captures maxCaptures FROM screen_capture_settings WHERE workspace_id=?').get(workspaceId) as Record<string, unknown> | undefined
    return row ? validateCaptureSettings({ mode: String(row.mode) as CaptureMode, shortcut: String(row.shortcut), retentionDays: Number(row.retentionDays) as 7|30|90, maxCaptures: Number(row.maxCaptures) }) : { mode: 'region', shortcut: defaultCaptureShortcut(process.platform), retentionDays: 30, maxCaptures: 100 }
  }
  ...
  setScreenCaptureSettings(workspaceId: string, settings: CaptureSettings): CaptureSettings {
    ...
    this.db.prepare('INSERT INTO screen_capture_settings VALUES (?,?,?,?,?,?) ON CONFLICT(workspace_id) DO UPDATE SET mode=excluded.mode,shortcut=excluded.shortcut,retention_days=excluded.retention_days,max_captures=excluded.max_captures,updated_at=excluded.updated_at').run(workspaceId, valid.mode, valid.shortcut, valid.retentionDays, valid.maxCaptures, now())
    this.activity(workspaceId, 'maintenance', 'screen_capture.settings_updated', workspaceId, 'workspace', { mode: valid.mode, shortcut: valid.shortcut, retentionDays: valid.retentionDays, maxCaptures: valid.maxCaptures })
