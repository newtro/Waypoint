...
export class WorkspaceStore {
  ...
  private materializeInboundDelete(workspaceId:string,objectId:string,kind:string,removeFiles:string[]):void{const attachments=this.db.prepare('SELECT relative_path FROM attachments WHERE workspace_id=? AND (id=? OR owner_id=?)').all(workspaceId,objectId,objectId) as Array<{relative_path:string}>;removeFiles.push(...attachments.map((item)=>item.relative_path));this.db.prepare('DELETE FROM attachments WHERE workspace_id=? AND (id=? OR owner_id=?)').run(workspaceId,objectId,objectId);this.db.prepare('DELETE FROM relationships WHERE workspace_id=? AND (id=? OR from_id=? OR to_id=?)').run(workspaceId,objectId,objectId,objectId);this.db.prepare('DELETE FROM embeddings WHERE workspace_id=? AND object_id=?').run(workspaceId,objectId);this.db.prepare('DELETE FROM search_fts WHERE workspace_id=? AND object_id=?').run(workspaceId,objectId);if(kind==='message'||kind==='any')this.db.prepare('DELETE FROM messages WHERE id=? AND EXISTS(SELECT 1 FROM chats WHERE id=messages.chat_id AND workspace_id=?)').run(objectId,workspaceId);const selected=kind==='document'?'documents':kind==='chat'?'chats':kind==='memory'?'memories':kind==='relationship'?'relationships':kind==='attachment'?'attachments':undefined,tables=kind==='any'?['documents','chats','memories','relationships','attachments']:selected?[selected]:[];for(const table of tables)this.db.prepare(`DELETE FROM ${table} WHERE id=? AND workspace_id=?`).run(objectId,workspaceId);this.db.prepare('INSERT OR REPLACE INTO tombstones VALUES (?,?,?,?)').run(objectId,workspaceId,kind,now())}
  ...
  deleteMessage(workspaceId:string,messageId:string):void{
    ...
    try{this.transaction(()=>{
      ...
      const cascadeIds=[...ownedIds,...attachments.map((item)=>item.id),...relationships];this.syncJournal.enqueue(workspaceId,messageId,'message','delete',{id:messageId,cascade:true,cascadeIds},cascadeIds)
  ...
  deleteObject(workspaceId: string, objectKind: 'document' | 'chat' | 'memory', objectId: string): void {
    ...
    try { this.transaction(() => {
      ...
      const cascadeIds=[...ownedIds,...relationshipIds,...attachmentRows.map((row)=>row.id)];this.syncJournal.enqueue(workspaceId,objectId,objectKind,'delete',{id:objectId,cascade:true,cascadeIds},cascadeIds)
