...
export class WorkspaceStore {
  ...
  voicePreferences(workspaceId:string){if(!this.db.prepare('SELECT 1 FROM workspaces WHERE id=?').get(workspaceId))throw new Error('Workspace not found');const row=this.db.prepare('SELECT mode,microphone_id microphoneId,output_voice outputVoice FROM voice_preferences WHERE workspace_id=?').get(workspaceId)as{mode:'push_to_talk'|'hands_free';microphoneId:string;outputVoice:'system'}|undefined;return row??{mode:'push_to_talk' as const,microphoneId:'',outputVoice:'system' as const}}
  setVoicePreferences(workspaceId:string,value:{mode:string;microphoneId:string;outputVoice:string}){if(!this.db.prepare('SELECT 1 FROM workspaces WHERE id=?').get(workspaceId))throw new Error('Workspace not found');if(!['push_to_talk','hands_free'].includes(value.mode)||value.microphoneId.length>512||value.outputVoice!=='system')throw new Error('Voice preferences are invalid');this.db.prepare('INSERT INTO voice_preferences VALUES (?,?,?,?,?) ON CONFLICT(workspace_id) DO UPDATE SET mode=excluded.mode,microphone_id=excluded.microphone_id,output_voice=excluded.output_voice,updated_at=excluded.updated_at').run(workspaceId,value.mode,value.microphoneId,value.outputVoice,now());return this.voicePreferences(workspaceId)}
