...
export class WorkspaceStore {
  ...
  private materializeInboundDelete(workspaceId: string, objectId: string, kind: string, removeFiles: string[]): void {
    ...
    const selected = kind === 'document' ? 'documents' : kind === 'chat' ? 'chats' : kind === 'memory' ? 'memories' : kind === 'relationship' ? 'relationships' : kind === 'attachment' ? 'attachments' : kind==='tool_failure'?'tool_failure_knowledge':kind==='provider_usage'?'provider_usage_receipts':undefined,
      tables = kind === 'any' ? ['documents', 'chats', 'memories', 'relationships', 'attachments','tool_failure_knowledge','provider_usage_receipts'] : selected ? [selected] : [];
  ...
  private assertInboundIdentityAvailable(workspaceId: string, id: string): void {
    for (const table of ['documents', 'chats', 'memories', 'relationships', 'attachments','tool_failure_knowledge','provider_usage_receipts']) {
  ...
  restoreWorkspace(archive: ExportArchive, newName: string, newLocalPath: string): WorkspaceSummary {
    ...
    try {
      ...
      this.ensureAutonomousDeveloperProfile(workspace.id, workspace.localPath);
        ...
        const restoredToolReceipts=new Map<string,string>();for(const value of archive.objects.tool_gateway_receipts??[]){const row=value as Record<string,unknown>,origin=String(row.origin),status=String(row.status),tool=String(row.tool),restoredId=randomUUID();if(!['ui','ai'].includes(origin)||!['completed','failed','canceled','timed_out','denied'].includes(status)||!['workspace.list_files','workspace.read_file','terminal.run','local_cli.run','waypoint.command'].includes(tool)||String(row.summary).length>500)throw new Error('Tool gateway receipt archive is invalid');this.db.prepare('INSERT INTO tool_gateway_receipts VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(restoredId,workspace.id,origin,tool,status,String(row.capability_version),'local',String(row.profile_name),String(row.policy_digest),String(row.summary),row.code==null?null:String(row.code),row.notification==null?null:String(row.notification),row.rollback_ref==null?null:String(row.rollback_ref),Number(row.output_bytes),Number(row.truncated)===1?1:0,String(row.started_at),String(row.finished_at),Number(row.duration_ms));restoredToolReceipts.set(String(row.id),restoredId)}
