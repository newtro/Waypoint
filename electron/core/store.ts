...
export class WorkspaceStore {
  ...
  private materializeInboundDelete(workspaceId: string, objectId: string, kind: string, removeFiles: string[]): void {
    ...
    const selected = kind === 'document' ? 'documents' : kind === 'chat' ? 'chats' : kind === 'memory' ? 'memories' : kind === 'relationship' ? 'relationships' : kind === 'attachment' ? 'attachments' : kind==='activity_snapshot'?'activity_snapshots':kind==='tool_failure'?'tool_failure_knowledge':kind==='provider_usage'?'provider_usage_receipts':undefined,
      tables = kind === 'any' ? ['documents', 'chats', 'memories', 'relationships', 'attachments','activity_snapshots','tool_failure_knowledge','provider_usage_receipts'] : selected ? [selected] : [];
  ...
  private assertInboundIdentityAvailable(workspaceId: string, id: string): void {
    for (const table of ['documents', 'chats', 'memories', 'relationships', 'attachments','activity_snapshots','tool_failure_knowledge','provider_usage_receipts']) {
  ...
  private canonicalObjectIds(workspaceId: string): string[] {
    const ids: string[] = [];
    for (const table of ['documents', 'chats', 'attachments','activity_snapshots']) ids.push(...(this.db.prepare(`SELECT id FROM ${table} WHERE workspace_id=?`).all(workspaceId) as Array<{ id: string }>).map((row) => row.id));
