...
describe("bounded local diagnostics", () => {
  it("returns stable, ordered pass results without external work", async () => {
    const probes = healthy(),
      report = await runDiagnostics(probes, "2026-08-02T00:00:00.000Z");
  ...
  it("classifies degraded, unavailable, and optional states with remediation", async () => {
    ...
    probes.storage = async () => ({
      freeBytes: 20,
      minimumFreeBytes: 1_000,
      writable: false,
    });
    probes.attachments = async () => ({
      missingFiles: 1,
      orphanFiles: 2,
      digestMismatches: 1,
    });
    probes.search = async () => ({ indexedObjects: 2, expectedObjects: 5 });
    probes.embeddings = async () => ({
      configured: true,
      reachable: false,
      model: "qwen",
      modelInstalled: false,
    });
    probes.cli = async (provider) =>
      provider === "codex"
        ? { configured: false }
    ...
    expect(
      results
        .filter(({ status }) => status === "blocked" || status === "warning")
        .every(({ remediation }) => Boolean(remediation)),
    ).toBe(true);
  });

  it("fails closed per probe and continues the remaining bounded checks", async () => {
    const probes = healthy(),
      cli = vi.fn(probes.cli!);
    probes.database = async () => {
      throw new TypeError("private /Users/person/workspace.sqlite");
    };
    probes.cli = cli;
    const results = (await runDiagnostics(probes)).results;
  ...
  it("marks omitted optional probes not configured", async () => {
    ...
    expect(results.every(({ status }) => status === "not_configured")).toBe(
      true,
    );
  });

  it("exports only minimized structured data and redacts path-like or sensitive detail fields", () => {
    const exported = exportDiagnosticsReport({
      formatVersion: 1,
      generatedAt: "fixed",
      results: [
        {
          ...
          status: "blocked",
          summary: "Cannot write locally.",
          remediation: "Fix permissions.",
          details: {
            freeBytes: 3,
            databasePath: "/Users/alice/private.sqlite",
            model: "/private/model",
            secretToken: "abc",
          },
        },
      ...
      results: [
        {
          ...
          summary: "Cannot write locally.",
          remediation: "Fix permissions.",
          details: { freeBytes: 3, model: "[redacted]" },
        },
      ],
    });
