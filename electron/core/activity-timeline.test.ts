...
describe('activity timeline privacy contract',()=>{
  ...
  it('keeps only content-minimized display metadata',()=>{
    expect(safeActivityDetails(JSON.stringify({cli:'codex',model:'gpt',device:'local',version:1,localPath:'/secret',prompt:'private',sourceMessageId:'id'}))).toEqual({cli:'codex',device:'local',version:1})
    expect(safeActivityDetails(JSON.stringify({status:'sk-proj-secretcredential',phase:'secretcredential',type:'secretcredential',extractor:'secretcredential',model:'secretcredential',cli:'malicious',created:Infinity}))).toEqual({})
