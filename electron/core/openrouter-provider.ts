...
export type OpenRouterSettings={enabled:boolean;liveRequestsEnabled:boolean;strategicModel:string;everydayModel:string;fallbackProvider?:SubscriptionProvider;monthlyCapMicros:number;ytdCapMicros:number;perRequestCapMicros?:number;warningPercent:number}
...
export function validateOpenRouterSettings(value:OpenRouterSettings):OpenRouterSettings{
  ...
  return{...value,perRequestCapMicros,strategicModel:value.strategicModel.trim(),everydayModel:value.everydayModel.trim()}
}
...
export interface OpenRouterTransport{complete(input:{apiKey:string;model:string;prompt:string;signal:AbortSignal;requestCapMicros:number;onProgress?:(value:{bytes:number})=>void}):Promise<{responseId:string;text:string;promptTokens:number;completionTokens:number;costMicros:number}>}
