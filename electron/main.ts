...
function registerIpc(): void {
  ...
  handle('waypoint:desktop-sync-complete-enrollment',async(_event,input:unknown)=>{const workspaceId=text((input as Record<string,unknown>).workspaceId,'workspace ID',64),result=await syncService.completeEnrollment(workspaceId);store.configureSyncDevice(workspaceId,result.deviceId);recordSyncActivityBestEffort(store,workspaceId,'device.enrolled');return result})
  ...
  handle('waypoint:desktop-sync-revoke',async(_event,input:unknown)=>{const value=input as Record<string,unknown>,workspaceId=text(value.workspaceId,'workspace ID',64),deviceId=text(value.deviceId,'device ID',64),confirmation=await dialog.showMessageBox({type:'warning',buttons:['Revoke and rotate','Cancel'],defaultId:1,cancelId:1,message:'Revoke this device?',detail:'The device will lose relay access immediately. Waypoint will rotate the workspace key for remaining devices.'});if(confirmation.response!==0)return{canceled:true};await syncService.revoke(workspaceId,deviceId);const rotation=await syncService.rotate(workspaceId);recordSyncActivityBestEffort(store,workspaceId,'device.revoked');recordSyncActivityBestEffort(store,workspaceId,'key.rotated');return{canceled:false,rotation}})
  handle('waypoint:desktop-sync-resume-rotation',async(_event,input:unknown)=>{const workspaceId=text((input as Record<string,unknown>).workspaceId,'workspace ID',64),result=await syncService.rotate(workspaceId);recordSyncActivityBestEffort(store,workspaceId,'key.rotated');return result})
  handle('waypoint:desktop-sync-now',async(_event,input:unknown)=>{const workspaceId=text((input as Record<string,unknown>).workspaceId,'workspace ID',64),result=await syncService.syncOnce(workspaceId,store);recordSyncActivityBestEffort(store,workspaceId,'sync.completed',{status:'completed'});return result})
  ...
  handle('waypoint:activity', (_event, input: unknown) => {const value=input as Record<string,unknown>,families=Array.isArray(value.families)?value.families.map((item)=>text(item,'activity family',20)) as import('./core/types.js').ActivityFamily[]:undefined,query=value.query===undefined?undefined:text(value.query,'activity query',100);return store.listActivity(text(value.workspaceId,'workspace ID',64),{families,query,limit:value.limit===undefined?undefined:Number(value.limit)})})
  handle('waypoint:create-meeting',(_event,input:unknown)=>{const value=input as Record<string,unknown>;if(value.consentAcknowledged!==true)throw new Error('Recording consent must be acknowledged for this session');return{meetingId:store.createMeeting(text(value.workspaceId,'workspace ID',64),text(value.title,'meeting title',300))}})
  handle('waypoint:finalize-meeting',(_event,input:unknown)=>{const value=input as Record<string,unknown>,workspaceId=text(value.workspaceId,'workspace ID',64),meetingId=text(value.meetingId,'meeting ID',64),mediaType=text(value.mediaType,'meeting media type',40);if(!(value.audio instanceof Uint8Array))throw new Error('Meeting audio payload is invalid');const bytes=Buffer.from(value.audio),disk=statfsSync(app.getPath('userData')),free=Number(disk.bavail)*Number(disk.bsize);if(free<bytes.length+256*1024*1024){store.failMeeting(workspaceId,meetingId,'disk_pressure');throw new Error('Not enough free space to save this recording')}try{store.finalizeMeetingAudio(workspaceId,meetingId,mediaType,bytes)}catch(error){store.failMeeting(workspaceId,meetingId,'capture_failed');throw error}return{ok:true}})
  handle('waypoint:fail-meeting',(_event,input:unknown)=>{const value=input as Record<string,unknown>,code=text(value.failureCode,'meeting failure code',40);if(!['permission_denied','device_lost','interrupted','disk_pressure','capture_failed','size_limit'].includes(code))throw new Error('Meeting failure code is invalid');store.failMeeting(text(value.workspaceId,'workspace ID',64),text(value.meetingId,'meeting ID',64),code as 'permission_denied'|'device_lost'|'interrupted'|'disk_pressure'|'capture_failed'|'size_limit');return{ok:true}})
  ...
  handle('waypoint:update-meeting-transcript',(_event,input:unknown)=>{const value=input as Record<string,unknown>;store.updateMeetingTranscript(text(value.workspaceId,'workspace ID',64),text(value.meetingId,'meeting ID',64),text(value.transcript,'meeting transcript',500_000),value.reviewed===true);return{ok:true}})
  handle('waypoint:save-meeting-memory',(_event,input:unknown)=>{const value=input as Record<string,unknown>;return{memoryId:store.saveMeetingTranscriptToMemory(text(value.workspaceId,'workspace ID',64),text(value.meetingId,'meeting ID',64))}})
  handle('waypoint:delete-meeting',(_event,input:unknown)=>{const value=input as Record<string,unknown>;store.deleteMeeting(text(value.workspaceId,'workspace ID',64),text(value.meetingId,'meeting ID',64));return{ok:true}})
  handle('waypoint:read-meeting-audio',(_event,input:unknown)=>{const value=input as Record<string,unknown>,audio=store.meetingAudio(text(value.workspaceId,'workspace ID',64),text(value.meetingId,'meeting ID',64));return{mediaType:audio.mediaType,audio:readFileSync(audio.path)}})
  handle('waypoint:export-meeting-audio',async(_event,input:unknown)=>{const value=input as Record<string,unknown>,audio=store.meetingAudio(text(value.workspaceId,'workspace ID',64),text(value.meetingId,'meeting ID',64)),extension=audio.mediaType==='audio/webm'?'webm':audio.mediaType==='audio/mp4'?'m4a':audio.mediaType==='audio/ogg'?'ogg':'wav',chosen=await dialog.showSaveDialog({title:'Export local meeting audio',defaultPath:`${audio.title.replace(/[^A-Za-z0-9._ -]/g,'_').slice(0,80)}.${extension}`});if(chosen.canceled||!chosen.filePath)return{canceled:true};writeFileSync(chosen.filePath,readFileSync(audio.path),{flag:'wx',mode:0o600});return{canceled:false}})
  handle('waypoint:meeting-transcription-capability',()=>({available:false,provider:'local-whisper',reason:'No reviewed packaged local transcription model is configured. Audio will not be uploaded or sent to a CLI. You can enter and review a transcript draft manually.'}))
...
function createWindow(): void {
  ...
  window.webContents.session.setPermissionRequestHandler((contents,permission,callback,details)=>{const trusted=contents.id===window.webContents.id&&details.requestingUrl===allowedUrl,mediaTypes='mediaTypes' in details&&Array.isArray(details.mediaTypes)?details.mediaTypes:[];callback(Boolean(trusted&&permission==='media'&&mediaTypes.length===1&&mediaTypes[0]==='audio'))})
