...
import{NativeSpeechAdapter,VoiceRuntimeRegistry}from'./core/voice-runtime.js';
...
let voiceRuntime:VoiceRuntimeRegistry,voicePacks:VoicePackManager;const voiceSpeech=new NativeSpeechAdapter(),voiceOperations=new VoiceOperationRegistry();type SpeechResult='completed'|'canceled'|'failed';let voiceSpeechOwner:{workspaceId:string;chatId:string;turnId:number;notify:(result:SpeechResult)=>void}|undefined;
...
function registerIpc(): void {
  ...
  handle('waypoint:voice-engine-status',(_event,input:unknown)=>{const workspaceId=text((input as Record<string,unknown>).workspaceId,'workspace ID',64),preference=store.voicePreferences(workspaceId),fast=voicePacks.status('fast_local'),experimental=voicePacks.status('full_duplex_experimental'),capability=voiceRuntime.capability();return{selected:preference.engine,engines:[{...fast,ready:capability.stt.available&&capability.tts.available,version:'compat-whisper-1.8.4/macos-say',packageBytes:capability.stt.source==='bundled'?60_000_000:0,reason:capability.stt.available&&capability.tts.available?'Bundled local transcription and interim native speech are ready; cross-platform Pocket TTS closure is still pending.':`${capability.stt.reason} ${capability.tts.reason}`,metrics:fixtureVoiceMetrics([{atMs:240,durationMs:80},{atMs:320,durationMs:80}])},{...experimental,metrics:experimental.metrics.fixture?experimental.metrics:fixtureVoiceMetrics([{atMs:650,durationMs:80},{atMs:730,durationMs:80}])}]}});
  ...
  handle('waypoint:tool-gateway-update-settings',(_event,input:unknown)=>{const value=input as Record<string,unknown>,workspaceId=text(value.workspaceId,'workspace ID',64),denyPatterns=Array.isArray(value.denyPatterns)?value.denyPatterns.map((item)=>text(item,'deny pattern',300)):[],next={stopped:value.stopped===true,denyPatterns,suppressCommit:value.suppressCommit===true,suppressPush:value.suppressPush===true};validatePolicy({...gatewayPolicy(workspaceId),...next});if(next.stopped){toolGateway.stop(workspaceId);store.cancelAllRemoteJobs(workspaceId);voiceOperations.stop(workspaceId);if(voiceSpeechOwner?.workspaceId===workspaceId){voiceSpeech.stop();voiceSpeechOwner=undefined}const capture=store.activityCapturePolicy(workspaceId);if(capture.enabled&&!capture.paused)store.setActivityCapturePolicy(workspaceId,{...capture,paused:true})}else toolGateway.resume(workspaceId);return store.setToolGatewaySettings(workspaceId,next)});
