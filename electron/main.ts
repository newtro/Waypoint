...
let voiceRuntime:VoiceRuntimeRegistry;const voiceSpeech=new NativeSpeechAdapter(),voiceOperations=new VoiceOperationRegistry();type SpeechResult='completed'|'canceled'|'failed';let voiceSpeechOwner:{workspaceId:string;chatId:string;turnId:number;notify:(result:SpeechResult)=>void}|undefined;
...
function registerIpc(): void {
  ...
  handle('waypoint:voice-update-preferences',(_event,input:unknown)=>{const value=input as Record<string,unknown>;return store.setVoicePreferences(text(value.workspaceId,'workspace ID',64),{mode:String(value.mode),microphoneId:String(value.microphoneId??''),outputVoice:String(value.outputVoice)})});
