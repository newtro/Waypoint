...
function registerIpc(): void {
  ...
  handle('waypoint:tool-gateway-update-settings',(_event,input:unknown)=>{const value=input as Record<string,unknown>,workspaceId=text(value.workspaceId,'workspace ID',64),denyPatterns=Array.isArray(value.denyPatterns)?value.denyPatterns.map((item)=>text(item,'deny pattern',300)):[],next={stopped:value.stopped===true,denyPatterns,suppressCommit:value.suppressCommit===true,suppressPush:value.suppressPush===true};validatePolicy({...gatewayPolicy(workspaceId),...next});if(next.stopped){toolGateway.stop(workspaceId);store.cancelAllRemoteJobs(workspaceId);voiceOperations.stop(workspaceId);if(voiceSpeechOwner?.workspaceId===workspaceId){const owner=voiceSpeechOwner;fastVoiceSpeech.stop();voiceSpeechOwner=undefined;for(const window of BrowserWindow.getAllWindows())window.webContents.send('waypoint:voice-audio-stop',{workspaceId,chatId:owner.chatId,turnId:owner.turnId});owner.notify('canceled')}const capture=store.activityCapturePolicy(workspaceId);if(capture.enabled&&!capture.paused)store.setActivityCapturePolicy(workspaceId,{...capture,paused:true})}else toolGateway.resume(workspaceId);return store.setToolGatewaySettings(workspaceId,next)});
