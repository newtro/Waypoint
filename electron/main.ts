...
import { accessSync, constants, lstatSync, mkdirSync, readFileSync, readdirSync, renameSync, rmSync, statfsSync, writeFileSync } from 'node:fs';
...
import{AGENT_BROWSER_VERSION,verifyBrowserClosure}from'./core/agent-browser.js';
...
let webSearchVault:ProtectedWebSearchVault;const controlledWebTools=new ControlledWebTools();
...
let voiceRuntime:VoiceRuntimeRegistry,voicePacks:VoicePackManager,fastVoiceSpeech:FastLocalSpeechProcessAdapter,fastVoiceTranscription:FastLocalTranscriptionProcessAdapter,meetingTranscription:FastLocalTranscriptionProcessAdapter,fastVoiceMetric:FastSpeechMetric|undefined,fastInterruptionMs:number|undefined,fastVoicePackageBytes=0;const voiceStopRequests=new Map<string,number>(),voiceOperations=new VoiceOperationRegistry(),meetingTranscriptionRuns=new Map<string,{workspaceId:string;meetingId:string;nextIndex:number;parts:string[];characters:number;controller:AbortController;inFlight:boolean;baseline:string;stopTimer:NodeJS.Timeout}>();type SpeechResult='completed'|'canceled'|'failed';let voiceSpeechOwner:{workspaceId:string;chatId:string;turnId:number;notify:(result:SpeechResult)=>void}|undefined;
function cancelMeetingRun(runId:string){const run=meetingTranscriptionRuns.get(runId);if(!run)return false;run.controller.abort();clearInterval(run.stopTimer);run.parts.length=0;meetingTranscriptionRuns.delete(runId);return true}
...
const activeRemoteExecutions=new Map<string,{workspaceId:string;runId:string}>();
let browserClosure:ReturnType<typeof verifyBrowserClosure>|undefined,browserClosureError='Browser closure has not been verified.';
...
async function processRemoteJobs(workspaceId:string){const sync=syncService.status(workspaceId);if(!sync.configured||!sync.deviceId||activeRemoteJobs.has(workspaceId))return;activeRemoteJobs.add(workspaceId);try{store.recoverRemoteJobs(workspaceId);const profile=store.listSecurityProfiles(workspaceId).find((item)=>item.name==='Autonomous developer'&&item.peerEligible),policy=store.deviceControlPolicy(workspaceId);if(!profile||!policy.enabled)return;let claimed:ReturnType<WorkspaceStore['claimRemoteJob']>;for(const capability of policy.allowedCapabilities){claimed=store.claimRemoteJob(workspaceId,sync.deviceId,sync.keyEpoch,remotePolicyDigest(capability));if(claimed)break}if(!claimed)return;store.startRemoteJob(workspaceId,claimed.job.id,claimed.leaseId);try{if(claimed.job.capability==='waypoint.workspace_summary'){const execution=await toolGateway.execute({version:1,workspaceId,origin:'ui',tool:'waypoint.command',arguments:{command:'workspace.summary',input:{}}},gatewayPolicy(workspaceId)),receipt=execution.result?.receipt;if(!receipt||receipt.status!=='completed')throw new Error(receipt?.code??'remote_domain_command_failed');store.finishRemoteJob(workspaceId,claimed.job.id,claimed.leaseId,'completed','Workspace summary completed on the selected device')}else{const cli=claimed.job.capability==='agent.codex'?'codex':'claude',capability=await detectCli(cli),runId=`remote-${claimed.job.id}`,events:string[]=[];if(!capability.available||!capability.executable)throw new Error(`${cli}_cli_unavailable`);const running=await workbench.start(runId,{cli,prompt:claimed.job.instruction,workspaceRoot:profile.roots[0],profile:{...profile,maxConcurrency:1,secretNames:[]},executable:capability.executable,version:capability.version,timeoutMs:Math.min(policy.maxDurationMs,claimed.job.timeoutMs),maxOutputBytes:262_144},(event)=>{if(event.type==='text'&&event.text)events.push(event.text)});activeRemoteExecutions.set(claimed.job.id,{workspaceId,runId});const terminal=await running.completion;activeRemoteExecutions.delete(claimed.job.id);if(terminal.status!=='completed')throw new Error(`remote_${cli}_${terminal.status}`);const summary=events.join('').trim().slice(0,1000);store.finishRemoteJob(workspaceId,claimed.job.id,claimed.leaseId,'completed',summary||`${cli} completed without a text result`)}}catch(error){activeRemoteExecutions.delete(claimed.job.id);try{store.finishRemoteJob(workspaceId,claimed.job.id,claimed.leaseId,'failed','Target device could not complete the bounded command',error instanceof Error?error.message:'remote_command_failed')}catch{/* cancellation or lease expiry won */}}}finally{activeRemoteJobs.delete(workspaceId)}}

function gatewayPolicy(workspaceId:string):ToolGatewayPolicy{
  const profile=store.listSecurityProfiles(workspaceId).find((item)=>item.name==='Autonomous developer');if(!profile)throw new Error('Autonomous developer profile is unavailable');const settings=store.toolGatewaySettings(workspaceId),environmentSecrets=Object.keys(process.env).filter((name)=>/(?:TOKEN|SECRET|PASSWORD|PASSWD|COOKIE|AUTH|API_KEY|PRIVATE_KEY|CREDENTIAL)/i.test(name)),closure=browserClosure,browserReady=Boolean(closure&&settings.browserProfileMode==='isolated'&&settings.browserAllowedDomains.length>0),policy:ToolGatewayPolicy={profileName:profile.name,roots:profile.roots,denyPatterns:settings.denyPatterns,stopped:settings.stopped,secretNames:[...new Set([...profile.secretNames,...environmentSecrets])],maxDurationMs:Math.min(120_000,profile.maxDurationMs),maxConcurrency:Math.min(4,profile.maxConcurrency),suppressCommit:settings.suppressCommit,suppressPush:settings.suppressPush,webFetchEnabled:settings.webFetchEnabled,webSearchEnabled:settings.webSearchEnabled&&webSearchVault.hasKey(),...(browserReady?{browserExecutable:closure!.agentBrowserExecutable,browserBrowserExecutable:closure!.browserExecutable,browserProfileMode:'isolated' as const,browserAllowedDomains:settings.browserAllowedDomains,browserSessionName:`workspace-${workspaceId}`,browserHomeDir:path.join(app.getPath('userData'),'browser-sessions',workspaceId)}:{})};validatePolicy(policy);return policy
}
...
function registerIpc(): void {
  ...
  handle('waypoint:tool-gateway-capabilities',()=>{const settings=store.openRouterSettings(),capability=openRouterCapability(settings,providerKeyConfigured(),store.providerUsage().summary);return{version:1,tools:toolGateway.descriptors().filter((item)=>item.name!=='agent_browser.run'||Boolean(browserClosure)),localClis:[discoverLocalCli('git'),discoverLocalCli('gh'),discoverLocalCli('az')],browser:{available:Boolean(browserClosure),backend:'Vercel Labs Agent Browser',version:AGENT_BROWSER_VERSION,profiles:['isolated'],reason:browserClosure?'Verified bundled interactive runtime for isolated sessions. Existing signed-in profile remains repair-required until a non-mutating contained import passes.':browserClosureError},web:{fetchAvailable:true,searchKeyConfigured:webSearchVault.hasKey(),searchProvider:'Brave Search API',reason:webSearchVault.hasKey()?'Search key is protected locally; each workspace must opt in before external requests are allowed.':'Add a Brave Search API key in Settings to make web search available. No background request is made.'},remoteProviders:{available:capability.available,provider:'openrouter',state:capability.state,health:capability.health,reason:capability.reason},crossDevice:{available:true,reason:'User-dispatched workspace summary jobs are available when encrypted sync and target worker policy are enabled.'}}});
  handle('waypoint:cross-workspace-rollup-settings',(_event,input:unknown)=>store.crossWorkspaceRollupSettings(text((input as Record<string,unknown>).workspaceId,'workspace ID',64)));
  handle('waypoint:cross-workspace-rollup-update',(_event,input:unknown)=>{const value=input as Record<string,unknown>,workspaceId=text(value.workspaceId,'workspace ID',64),body=value.value as Record<string,unknown>,grants=Array.isArray(body.grants)?body.grants.map((grant)=>{const item=grant as Record<string,unknown>,family=String(item.family);if(!['commitments','meetings','briefing_status'].includes(family))throw new Error('Roll-up family is invalid');return{sourceWorkspaceId:text(item.sourceWorkspaceId,'source workspace ID',64),family:family as 'commitments'|'meetings'|'briefing_status',enabled:item.enabled===true}}):[];return store.setCrossWorkspaceRollupSettings(workspaceId,{standingEnabled:body.standingEnabled===true,grants})});
  handle('waypoint:cross-workspace-rollup-compose',(_event,input:unknown)=>{const value=input as Record<string,unknown>,families=Array.isArray(value.families)?value.families.map(String).filter((item):item is 'commitments'|'meetings'|'briefing_status'=>['commitments','meetings','briefing_status'].includes(item)):undefined;return store.composeCrossWorkspaceRollup(text(value.workspaceId,'workspace ID',64),families)});
  handle('waypoint:web-search-set-key',(_event,input:unknown)=>{webSearchVault.setKey(text((input as Record<string,unknown>).apiKey,'Brave Search API key',512));return{keyConfigured:true}});
  handle('waypoint:web-search-remove-key',()=>{webSearchVault.removeKey();for(const workspace of store.listWorkspaces()){const current=store.toolGatewaySettings(workspace.id);if(current.webSearchEnabled)store.setToolGatewaySettings(workspace.id,{...current,webSearchEnabled:false})}return{keyConfigured:false}});
  ...
  handle('waypoint:web-tools-update',(_event,input:unknown)=>{const value=input as Record<string,unknown>,workspaceId=text(value.workspaceId,'workspace ID',64),current=store.toolGatewaySettings(workspaceId);return store.setToolGatewaySettings(workspaceId,{...current,webFetchEnabled:value.webFetchEnabled===true,webSearchEnabled:value.webSearchEnabled===true&&webSearchVault.hasKey()})});
  handle('waypoint:tool-gateway-update-settings',async(_event,input:unknown)=>{const value=input as Record<string,unknown>,workspaceId=text(value.workspaceId,'workspace ID',64),current=store.toolGatewaySettings(workspaceId),priorPolicy=gatewayPolicy(workspaceId),denyPatterns=Array.isArray(value.denyPatterns)?value.denyPatterns.map((item)=>text(item,'deny pattern',300)):[],browserAllowedDomains=Array.isArray(value.browserAllowedDomains)?value.browserAllowedDomains.map((item)=>text(item,'browser domain',253)):[],next={stopped:value.stopped===true,denyPatterns,suppressCommit:value.suppressCommit===true,suppressPush:value.suppressPush===true,browserProfileMode:value.browserProfileMode==='existing'?'existing' as const:'isolated' as const,browserProfileName:text(value.browserProfileName??'Default','browser profile',100),browserAllowedDomains};validatePolicy({...priorPolicy,...next});const browserAuthorityChanged=current.browserProfileMode!==next.browserProfileMode||current.browserProfileName!==next.browserProfileName||JSON.stringify(current.browserAllowedDomains)!==JSON.stringify(next.browserAllowedDomains);if(next.stopped||browserAuthorityChanged){const closed=await toolGateway.stopAndCloseBrowser(workspaceId,priorPolicy);if(!closed&&browserClosure)throw new Error('Browser session could not be proven closed; policy was not changed')}if(next.stopped){const reflectionRun=activeReflectionRuns.get(workspaceId);if(reflectionRun){killedReflectionWorkspaces.add(workspaceId);workbench.cancel(`reflection-${reflectionRun}`)}store.cancelAllRemoteJobs(workspaceId);voiceOperations.stop(workspaceId);if(voiceSpeechOwner?.workspaceId===workspaceId){const owner=voiceSpeechOwner;fastVoiceSpeech.stop();voiceSpeechOwner=undefined;for(const window of BrowserWindow.getAllWindows())window.webContents.send('waypoint:voice-audio-stop',{workspaceId,chatId:owner.chatId,turnId:owner.turnId});owner.notify('canceled')}const capture=store.activityCapturePolicy(workspaceId);if(capture.enabled&&!capture.paused)store.setActivityCapturePolicy(workspaceId,{...capture,paused:true})}else toolGateway.resume(workspaceId);return store.setToolGatewaySettings(workspaceId,next)});
  handle('waypoint:tool-gateway-clear-browser-data',async(_event,input:unknown)=>{const workspaceId=text((input as Record<string,unknown>).workspaceId,'workspace ID',64),target=path.join(app.getPath('userData'),'browser-sessions',workspaceId),closed=await toolGateway.stopAndCloseBrowser(workspaceId,gatewayPolicy(workspaceId));if(!closed&&browserClosure)throw new Error('Browser session could not be proven closed; isolated data was not cleared');rmSync(target,{recursive:true,force:true});mkdirSync(path.dirname(target),{recursive:true,mode:0o700});if(!store.toolGatewaySettings(workspaceId).stopped)toolGateway.resume(workspaceId);return{cleared:true}});
  ...
  handle('waypoint:device-control-status',async(_event,input:unknown)=>{const workspaceId=text((input as Record<string,unknown>).workspaceId,'workspace ID',64),[codex,claude]=await Promise.all([detectCli('codex'),detectCli('claude')]);return{policy:store.deviceControlPolicy(workspaceId),jobs:store.listRemoteJobs(workspaceId),sync:syncService.status(workspaceId),capabilities:[{id:'waypoint.workspace_summary',available:true,label:'Workspace summary'},{id:'agent.codex',available:codex.available,label:'Codex agent',reason:codex.available?'Bounded target-local signed-in CLI delegation is ready.':'Codex CLI is unavailable on this device.'},{id:'agent.claude',available:claude.available,label:'Claude agent',reason:claude.available?'Bounded target-local signed-in CLI delegation is ready.':'Claude CLI is unavailable on this device.'}]}});
  ...
  handle('waypoint:device-control-dispatch',(_event,input:unknown)=>{const value=input as Record<string,unknown>,workspaceId=text(value.workspaceId,'workspace ID',64),targetDeviceId=text(value.targetDeviceId,'target device ID',128),instruction=text(value.instruction,'remote instruction',8000),capability=['agent.codex','agent.claude'].includes(String(value.capability))?String(value.capability) as 'agent.codex'|'agent.claude':'waypoint.workspace_summary',sync=syncService.status(workspaceId);if(!sync.configured||!sync.deviceId)throw new Error('Device sync is not configured');return store.createRemoteJobRecord({workspaceId,controllerDeviceId:sync.deviceId,targetDeviceId,capability,instruction,idempotencyKey:text(value.idempotencyKey,'idempotency key',128),profileDigest:remotePolicyDigest(capability),keyEpoch:sync.keyEpoch,timeoutMs:60_000})});
  handle('waypoint:device-control-cancel',(_event,input:unknown)=>{const value=input as Record<string,unknown>,workspaceId=text(value.workspaceId,'workspace ID',64),jobId=text(value.jobId,'remote job ID',128),active=activeRemoteExecutions.get(jobId);if(active?.workspaceId===workspaceId)workbench.cancel(active.runId);return{canceled:store.cancelRemoteJob(workspaceId,jobId)}});
  ...
  handle('waypoint:delete-meeting', (_event, input: unknown) => {
    const value = input as Record<string, unknown>;
    const workspaceId=text(value.workspaceId,'workspace ID',64),meetingId=text(value.meetingId,'meeting ID',64);for(const[id,run]of meetingTranscriptionRuns)if(run.workspaceId===workspaceId&&run.meetingId===meetingId)cancelMeetingRun(id);
  ...
  handle('waypoint:meeting-transcription-capability',async()=>{const available=await meetingTranscription.probe();return{available,provider:'Fast Local Whisper tiny.en',speakerDiarization:false,reason:available?'Packaged local English transcription is ready for recordings up to ten minutes. Output is an unreviewed draft; speaker identity and diarization are not inferred. Audio never leaves this device.':'Packaged local transcription failed its readiness probe. Manual transcript review remains available; no cloud fallback will be used.'}});
  handle('waypoint:meeting-transcription-start',(event,input:unknown)=>{const value=input as Record<string,unknown>,workspaceId=text(value.workspaceId,'workspace ID',64),meetingId=text(value.meetingId,'meeting ID',64),meeting=store.listMeetings(workspaceId).find((item)=>item.id===meetingId);if(!meeting||meeting.status!=='ready')throw new Error('Completed meeting not found');store.meetingAudio(workspaceId,meetingId);if(meetingTranscriptionRuns.size)throw new Error('A local meeting transcription is already running');const runId=randomUUID(),controller=new AbortController(),deadline=Date.now()+15*60_000,stopTimer=setInterval(()=>{if(Date.now()>deadline||store.toolGatewaySettings(workspaceId).stopped)cancelMeetingRun(runId)},100);stopTimer.unref();event.sender.once('destroyed',()=>cancelMeetingRun(runId));meetingTranscriptionRuns.set(runId,{workspaceId,meetingId,nextIndex:0,parts:[],characters:0,controller,inFlight:false,baseline:JSON.stringify([meeting.transcript,meeting.transcriptStatus]),stopTimer});return{runId}});
  handle('waypoint:meeting-transcription-segment',async(_event,input:unknown)=>{const value=input as Record<string,unknown>,runId=text(value.runId,'transcription run ID',64),workspaceId=text(value.workspaceId,'workspace ID',64),meetingId=text(value.meetingId,'meeting ID',64),run=meetingTranscriptionRuns.get(runId),index=Number(value.index);if(!run||run.workspaceId!==workspaceId||run.meetingId!==meetingId||run.inFlight||!Number.isSafeInteger(index)||index!==run.nextIndex||index>=5||!(value.audio instanceof Uint8Array)||value.audio.byteLength>12*1024*1024)throw new Error('Meeting transcription segment is invalid');if(store.toolGatewaySettings(run.workspaceId).stopped){cancelMeetingRun(runId);throw new Error('Global stop is active')}run.inFlight=true;try{const result=await meetingTranscription.transcribe(value.audio,run.controller.signal),part=result.text.slice(0,100_000);if(run.controller.signal.aborted||!meetingTranscriptionRuns.has(runId))throw new Error('Meeting transcription canceled');if(run.characters+part.length>500_000)throw new Error('Meeting transcript exceeds bounds');run.parts.push(part);run.characters+=part.length;run.nextIndex++;return{completedSegments:run.nextIndex}}finally{run.inFlight=false;value.audio.fill(0)}});
  handle('waypoint:meeting-transcription-finish',(_event,input:unknown)=>{const value=input as Record<string,unknown>,runId=text(value.runId,'transcription run ID',64),workspaceId=text(value.workspaceId,'workspace ID',64),meetingId=text(value.meetingId,'meeting ID',64),run=meetingTranscriptionRuns.get(runId),meeting=store.listMeetings(workspaceId).find((item)=>item.id===meetingId);if(!run||run.workspaceId!==workspaceId||run.meetingId!==meetingId||run.inFlight||!run.parts.length||!meeting||meeting.status!=='ready'||store.toolGatewaySettings(workspaceId).stopped||run.baseline!==JSON.stringify([meeting.transcript,meeting.transcriptStatus])){cancelMeetingRun(runId);throw new Error('Meeting transcription could not be committed because its source or policy changed')}clearInterval(run.stopTimer);meetingTranscriptionRuns.delete(runId);const transcript=run.parts.join('\n\n');run.parts.length=0;store.updateMeetingTranscript(run.workspaceId,run.meetingId,transcript,false);return{transcript,provider:'Fast Local Whisper tiny.en'}});
  handle('waypoint:meeting-transcription-cancel',(_event,input:unknown)=>{const value=input as Record<string,unknown>,runId=text(value.runId,'transcription run ID',64),workspaceId=text(value.workspaceId,'workspace ID',64),meetingId=text(value.meetingId,'meeting ID',64),run=meetingTranscriptionRuns.get(runId);return{canceled:Boolean(run&&run.workspaceId===workspaceId&&run.meetingId===meetingId&&cancelMeetingRun(runId))}});
...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  app.whenReady().then(() => {
    ...
    try{browserClosure=verifyBrowserClosure(app.isPackaged?path.join(process.resourcesPath,'agent-browser'):path.resolve(currentDirectory,'../../vendor/browser-staging'));browserClosureError=''}catch(error){browserClosure=undefined;browserClosureError=`Unavailable: ${error instanceof Error?error.message:'browser_integrity_failed'}`}
    ...
    webSearchVault=new ProtectedWebSearchVault(app.getPath('userData'),{available:()=>safeStorage.isEncryptionAvailable(),encrypt:(value)=>safeStorage.encryptString(value),decrypt:(value)=>safeStorage.decryptString(value)});
