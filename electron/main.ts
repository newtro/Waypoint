...
import{AGENT_BROWSER_VERSION}from'./core/agent-browser.js';
...
function registerIpc(): void {
  ...
  handle('waypoint:tool-gateway-capabilities',()=>{const settings=store.openRouterSettings(),capability=openRouterCapability(settings,providerKeyConfigured(),store.providerUsage().summary);return{version:1,tools:toolGateway.descriptors().filter((item)=>item.name!=='agent_browser.run'),localClis:[discoverLocalCli('git'),discoverLocalCli('gh'),discoverLocalCli('az')],browser:{available:false,backend:'Vercel Labs Agent Browser',version:AGENT_BROWSER_VERSION,profiles:[],reason:'Unavailable: browser closure, integrity probe, user-owned domain policy, and safe profile containment have not passed security review.'},remoteProviders:{available:capability.available,provider:'openrouter',state:capability.state,health:capability.health,reason:capability.reason},crossDevice:{available:true,reason:'User-dispatched workspace summary jobs are available when encrypted sync and target worker policy are enabled.'}}});
