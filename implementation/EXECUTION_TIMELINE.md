...
- Visual polish follow-up inspected the real normal-profile package before/after, replaced the utility-style capture surfaces with a cohesive viewfinder/source/preview/editor workbench, added responsive capture Settings and keyboard-equivalent annotation controls, and repaired history annotation while a chooser is open. Full gate: 117 suites / 514 tests, 0 lint errors, package/runtime closure, and final independent re-review 0 blocker / 0 high. Evidence: `implementation/P11_SCREEN_CAPTURE_POLISH_EVIDENCE.md`.
- Urgent black-frame repair reproduced the user's persisted 7568×4320 `Window → Waypoint` all-black PNG and isolated the cause to hiding the selected Waypoint BrowserWindow before Electron's final recapture. Window capture now hides only the local overlay, uses source-enumeration dimensions, and rejects the exact all-zero RGB frame signature before persistence. The repaired normal-profile package captured visible Waypoint pixels, flattened an annotation, and reopened the visible saved PNG from history. Evidence: `implementation/P11_BLACK_CAPTURE_REPAIR_EVIDENCE.md`.
- Annotation-editor viewport repair reproduced the missing footer at 1180×650 and replaced competing height calculations with a five-row editor grid. Tools/properties remain horizontally discoverable, only the canvas scrolls vertically, and Discard/Save/Done stay fixed above the safe area. Packaged normal-profile measurements cover short, narrow practical minimum, and fullscreen layouts. Evidence: `implementation/P11_EDITOR_VIEWPORT_REPAIR_EVIDENCE.md`.
# 2026-08-21 — Device invitation, Settings theme, and model thinking repair

- Reproduced the packaged desktop-host invitation failure at the exact runtime boundary: Electron's embedded OpenSSL rejected the stored directly pinned self-signed leaf, while standalone Node accepted the same path. Kept exact certificate pinning and enabled partial-chain trust only for that certificate.
- Implemented host-start confirmation, a live preflight status check, bounded registration, visible one-use invitation lifecycle, duplicate-click/workspace guards, stale-state recovery, and clipboard-denial recovery.
- Added provider/model-aware thinking controls and transport propagation for Codex, Claude, Grok Build, and OpenRouter, plus device-local persistence and deletion cascade.
- Repaired a package-closure failure by explicitly including the emitted shared thinking contract in `app.asar`. The rebuilt normal-profile macOS package launched, produced a real one-use desktop-host invitation, stopped cleanly afterward, rendered correct light/dark controls, and reflowed without horizontal overflow at a 900×650 viewport.
- Independent review drove repairs for active-turn thinking identity, shared provider capability validation, abandoned-workspace host cleanup, complete Settings control theming, atomic OpenRouter model/thinking/budget drafts, and stale asynchronous Settings refreshes. Final severity verdict: 0 blocker / 0 high / 0 medium / 0 low.
- Terminal gate passed the pinned Node 24.15.0/npm 12.0.1 toolchain, 163 test files / 826 passing / 3 platform skips, zero-warning lint, TypeScript/Vite build, Product Help preparation, macOS arm64 package and strict ad-hoc signature verification, diff hygiene, and production-path packaged Fast Local closure (312 ms first playable audio at 24 kHz). Release signing/notarization and physical Windows verification remain separate identity/hardware gates.
