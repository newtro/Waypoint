...
Package closure first runs the same packaged Fast Local readiness/model probe that the app runs when a workspace loads, then measures a fresh one-shot worker from process launch through complete first-segment audio receipt. The non-representative shared GitHub runners use a strict three-second closure ceiling and print the measured value; this catches missing, stalled, or grossly regressed native runtime behavior without representing shared-runner speed as product latency. Waypoint's product acceptance target remains at most one second on representative user hardware, and local/default package verification continues to enforce that target. Windows user-hardware voice latency remains a physical acceptance item rather than a release-automation claim.
