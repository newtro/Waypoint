...
export interface RelayConfig{host:string;port:number;databasePath:string;authorityRegistryPath:string;tlsMode:'proxy-loopback';logLevel:'error'|'warn'|'info'}
export function parseRelayConfig(input:Record<string,string|undefined>):RelayConfig{
  const port=Number(input.WAYPOINT_RELAY_PORT??'8443'),host=input.WAYPOINT_RELAY_HOST??'127.0.0.1',databasePath=input.WAYPOINT_RELAY_DATABASE??'',authorityRegistryPath=input.WAYPOINT_RELAY_AUTHORITY_REGISTRY??'',tlsMode=input.WAYPOINT_RELAY_TLS_MODE,logLevel=input.WAYPOINT_RELAY_LOG_LEVEL??'info'
  ...
  if(!path.isAbsolute(databasePath)||!path.isAbsolute(authorityRegistryPath))throw new Error('Relay database and authority registry paths must be absolute')
  ...
  return{host,port,databasePath,authorityRegistryPath,tlsMode,logLevel:logLevel as RelayConfig['logLevel']}
}
