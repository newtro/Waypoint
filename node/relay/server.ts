...
import {WebhookRelayService,WEBHOOK_LIMITS} from './webhook-service.js'
export {canonicalRelayRequest} from '../../electron/core/sync/relay-request.js'

const MAX_BODY=9*1024*1024,ID=/^[A-Za-z0-9_-]{16,128}$/
async function readBody(request:IncomingMessage,limit=MAX_BODY):Promise<Buffer>{const chunks:Buffer[]=[];let size=0;for await(const chunk of request){const value=Buffer.from(chunk);size+=value.length;if(size>limit)throw new Error('request_too_large');chunks.push(value)}return Buffer.concat(chunks)}
...
export async function createRelayServer(config:RelayConfig):Promise<{server:Server;relay:DurableOpaqueRelayService}>{
  ...
  const relay=new DurableOpaqueRelayService(config.databasePath,authority),webhooks=new WebhookRelayService(config.databasePath,readFileSync(config.webhookKeyPath)),principals=new Map<string,{start:number;count:number}>(),hooks=new Map<string,{start:number;count:number}>(),global=new Map<string,{start:number;count:number}>(),healthRate=new Map<string,{start:number;count:number}>();let inFlight=0,cachedHealth={...relay.health(),...webhooks.stats()};const healthTimer=setInterval(()=>{cachedHealth={...relay.health(),...webhooks.stats()}},60_000);healthTimer.unref()
  const server=createServer(async(request,response)=>{
    ...
    try{
      ...
      const hookMatch=/^\/v1\/hooks\/([A-Za-z0-9_-]{16,128})$/.exec(url.pathname);if(method==='POST'&&hookMatch){if(!allowed(hooks,hookMatch[1],started,WEBHOOK_LIMITS.maxRequestsPerMinute)){json(response,429,{error:'rate_limited'});return}const body=await readBody(request,WEBHOOK_LIMITS.maxBodyBytes),result=webhooks.accept(hookMatch[1],{timestamp:String(request.headers['x-waypoint-timestamp']??''),nonce:String(request.headers['x-waypoint-nonce']??''),signature:String(request.headers['x-waypoint-signature']??''),secretVersion:Number(request.headers['x-waypoint-secret-version']),mime:String(request.headers['content-type']??'')},body);json(response,202,result);return}
      ...
      if(method==='POST'&&url.pathname==='/v1/webhook-channels'){if(!authority.isOwner(workspaceId,deviceId)){json(response,403,{error:'owner_required'});return}const value=JSON.parse(body.toString('utf8')) as {label:string},recipient=authority.identity(workspaceId,deviceId);json(response,201,webhooks.create(workspaceId,deviceId,recipient!.encryptionPublicKey,String(value.label)));return}
      if(method==='GET'&&url.pathname==='/v1/webhook-channels'){if(!authority.isOwner(workspaceId,deviceId)){json(response,403,{error:'owner_required'});return}json(response,200,{channels:webhooks.list(workspaceId),killSwitch:webhooks.killed(workspaceId)});return}
      const channelAction=/^\/v1\/webhook-channels\/([A-Za-z0-9_-]{16,128})\/(rotate|revoke|delete)$/.exec(url.pathname);if(method==='POST'&&channelAction){if(!authority.isOwner(workspaceId,deviceId)){json(response,403,{error:'owner_required'});return}const [,channelId,action]=channelAction;json(response,200,action==='rotate'?webhooks.rotate(workspaceId,channelId):action==='revoke'?webhooks.revoke(workspaceId,channelId):{deleted:webhooks.deleteChannel(workspaceId,channelId)});return}
      if(method==='POST'&&url.pathname==='/v1/webhook-kill'){if(!authority.isOwner(workspaceId,deviceId)){json(response,403,{error:'owner_required'});return}const value=JSON.parse(body.toString('utf8')) as {active:boolean};if(typeof value.active!=='boolean')throw new Error('invalid_request');json(response,200,webhooks.setKill(workspaceId,value.active));return}
      if(method==='GET'&&url.pathname.startsWith('/v1/webhook-events')){const limit=Number(url.searchParams.get('limit')??50);json(response,200,{events:webhooks.pull(workspaceId,deviceId,limit)});return}
      if(method==='POST'&&url.pathname==='/v1/webhook-acks'){const value=JSON.parse(body.toString('utf8')) as {eventId:string};json(response,200,{acknowledged:webhooks.acknowledge(workspaceId,deviceId,String(value.eventId))});return}
    ...
    }catch(error){const message=error instanceof Error?error.message:'invalid_request',code=message==='request_too_large'?413:message==='unauthorized'||message==='replay'?401:message==='quota'?429:400;json(response,code,{error:code===413?'request_too_large':code===401?'unauthorized':code===429?'rate_limited':'invalid_request'})}finally{inFlight--}
  })
  server.maxHeadersCount=32;server.headersTimeout=10_000;server.requestTimeout=15_000;server.keepAliveTimeout=5_000;server.on('close',()=>{clearInterval(healthTimer);webhooks.close();relay.close();authority.close()});return{server,relay}
}
