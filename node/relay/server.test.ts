...
describe('hosted authenticated relay',()=>{
  ...
  it('binds signed principals, persists nonce replay defense, and round-trips encrypted peer traffic',async()=>{
    await sodium.ready
    const root=mkdtempSync(path.join(tmpdir(),'waypoint-hosted-')),database=path.join(root,'relay.sqlite'),registry=path.join(root,'authority.json'),webhookKey=path.join(root,'webhook.key'),crypto=await WaypointCrypto.create(),a=crypto.generateDevice('opaque_device_a_01'),b=crypto.generateDevice('opaque_device_b_01'),workspaceId='opaque_workspace_01',workspaceKey=crypto.generateWorkspaceKey()
    ...
    writeFileSync(webhookKey,randomBytes(32));const config={host:'127.0.0.1',port:8789,databasePath:database,authorityRegistryPath:registry,webhookKeyPath:webhookKey,tlsMode:'proxy-loopback' as const,logLevel:'info' as const},instance=await createRelayServer(config)
  ...
  it('completes signed one-use HTTP enrollment with fresh peer proof',async()=>{
    const root=mkdtempSync(path.join(tmpdir(),'waypoint-enroll-http-')),database=path.join(root,'relay.sqlite'),registry=path.join(root,'authority.json'),webhookKey=path.join(root,'webhook.key'),crypto=await WaypointCrypto.create(),owner=crypto.generateDevice('opaque_owner_http01'),peer=crypto.generateDevice('opaque_peer_http_01'),workspaceId='opaque_workspace_http01',workspaceKey=crypto.generateWorkspaceKey()
    ...
    writeFileSync(webhookKey,randomBytes(32));const instance=await createRelayServer({host:'127.0.0.1',port:8789,databasePath:database,authorityRegistryPath:registry,webhookKeyPath:webhookKey,tlsMode:'proxy-loopback',logLevel:'info'});running.push(instance.server);await new Promise<void>((resolve)=>instance.server.listen(0,'127.0.0.1',resolve));const address=instance.server.address();if(!address||typeof address==='string')throw new Error('missing address');const base=`http://127.0.0.1:${address.port}`
