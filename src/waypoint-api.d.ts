...
declare global {
  interface Window {
    waypoint: {
      ...
      updateScreenCaptureSettings(workspaceId:string,settings:{workflow:'guided'|'quick';mode:'region'|'window'|'display';shortcut:string;retentionDays:7|30|90;maxCaptures:number}):Promise<{workflow:'guided'|'quick';mode:'region'|'window'|'display';shortcut:string;retentionDays:7|30|90;maxCaptures:number;shortcutReady:boolean;shortcutReason:string}>;
      setScreenCaptureShortcutRecording(workspaceId:string,active:boolean):Promise<{registered:boolean;shortcut:string;reason:string}>;
      ...
      updateOpenRouterSettings(value:{enabled:boolean;liveRequestsEnabled:boolean;strategicModel:string;everydayModel:string;attachmentModel:string;fallbackProvider?:'codex'|'claude';monthlyCapMicros:number;ytdCapMicros:number;perRequestCapMicros:number;warningPercent:number}):Promise<{enabled:boolean;liveRequestsEnabled:boolean;strategicModel:string;everydayModel:string;attachmentModel:string;fallbackProvider?:'codex'|'claude';monthlyCapMicros:number;ytdCapMicros:number;perRequestCapMicros:number;warningPercent:number}>;
      runOpenRouterChat(value:{workspaceId:string;chatId:string;sourceMessageId:string;prompt:string;role:'strategic'|'everyday';securityProfileId:string;attachmentIds:string[]}):Promise<{runId?:string;status?:'running';model?:string;fallbackProvider?:'codex'|'claude';reason?:string;attachmentDelivery?:{delivered:string[];mode:'image-and-local-text'|'local-text'|'none'}}>;
