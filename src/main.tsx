...
export function App() {
  ...
  async function refresh(next = workspace) {
    ...
    const [nextChats, nextDocuments, nextMemories, nextSuggestions, nextCommitments, nextActivity, nextProfiles, nextRuns, nextSync, nextDesktop,nextChatModels] = await Promise.all([window.waypoint.listChats(next.id), window.waypoint.listDocuments(next.id), window.waypoint.listMemories(next.id), window.waypoint.listMemorySuggestions(next.id), window.waypoint.listCommitments(next.id), window.waypoint.activity(next.id, { limit: 500 }), window.waypoint.listSecurityProfiles(next.id), window.waypoint.listExecutions(next.id), window.waypoint.syncStatus(next.id), window.waypoint.desktopSyncStatus(next.id),window.waypoint.chatModelPreferences(next.id)]);
          ...
          <>
            ...
            <div className="composer-dock">
              <form className="composer" onSubmit={(event) => void runChat(event, selectedChat.id)}>
                {voiceOpen&&<section className="voice-mode" aria-label="Voice chat controls"><header><div><strong>Voice chat · {voiceState.replace('_',' ')}</strong><small role="status" aria-live="polite">{voicePartial||voiceCapability?.stt.reason||'Checking bundled local speech…'}</small></div><button type="button" className="quiet-button" onClick={()=>void stopVoiceMode().then(()=>setVoiceOpen(false))} aria-label="Close and stop voice chat">×</button></header><p>Microphone audio stays ephemeral and local. Waypoint asks for permission only when you start. Headphones reduce echo. This is turn-based voice, not full duplex.</p><div className="voice-settings"><label>Mode<select value={voiceMode} onChange={(event)=>setVoiceMode(event.target.value as VoiceMode)} disabled={voiceState!=='off'&&voiceState!=='error'}><option value="push_to_talk">Push to talk</option><option value="hands_free">Hands-free session</option></select></label><label>Microphone<select value={voiceDevice} onChange={(event)=>setVoiceDevice(event.target.value)} disabled={voiceState!=='off'&&voiceState!=='error'}><option value="">System default</option>{voiceDevices.map((device,index)=><option value={device.deviceId} key={device.deviceId||index}>{device.label||`Microphone ${index+1}`}</option>)}</select></label></div>{!voiceCapability?.stt.available?<div className="voice-readiness" role="alert"><strong>Bundled local transcription unavailable</strong><span>{voiceCapability?.stt.reason}</span></div>:<div className="voice-actions">{voiceMode==='push_to_talk'?<button type="button" className="voice-primary" disabled={voiceState==='transcribing'} onPointerDown={()=>void startVoiceCapture()} onPointerUp={()=>void finishVoiceCapture()} onPointerCancel={()=>void stopVoiceMode()} onKeyDown={(event)=>{if(!event.repeat&&(event.key===' '||event.key==='Enter')){event.preventDefault();void startVoiceCapture()}}} onKeyUp={(event)=>{if(event.key===' '||event.key==='Enter'){event.preventDefault();void finishVoiceCapture()}}}>{voiceState==='listening'?'Release to send':voiceState==='thinking'||voiceState==='speaking'?'Hold to interrupt':'Hold to talk'}</button>:<button type="button" className="voice-primary" disabled={voiceState==='transcribing'} onClick={()=>void toggleHandsFree()}>{voiceState==='listening'?'Finish turn':voiceState==='thinking'||voiceState==='speaking'?'Interrupt and listen':'Start hands-free'}</button>}<button type="button" onClick={()=>void stopVoiceMode()} disabled={voiceState==='off'}>Stop now</button></div>}<small>{voiceCapability?.tts.available?'Replies use the local macOS system voice.':'Speech playback is unavailable on this platform.'} Raw microphone audio is never added to chat, backup, sync, or activity.</small></section>}
                {queued.length > 0 && (
