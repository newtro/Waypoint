...
export function App() {
  ...
  const voiceCaptureRef=useRef(new BrowserPcmCapture()),voiceTurnRef=useRef(0),voiceSubmissionRef=useRef<number|undefined>(undefined),voiceRunRef=useRef<{turn:number;workspaceId?:string;chatId:string;sourceMessageId?:string;runId?:string;spoken?:boolean}|undefined>(undefined),voiceStateRef=useRef<VoiceState>('off'),voicePressReleasedRef=useRef(false),voiceCaptureTargetRef=useRef<{workspaceId:string;chatId:string}|undefined>(undefined),voiceScopeRef=useRef<{workspaceId?:string;chatId?:string}>({});
  ...
  async function refresh(next = workspace) {
    ...
    setVoiceMode(nextVoice.mode);setVoiceDevice(nextVoice.microphoneId);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(()=>window.waypoint.onVoiceSpeechState((event)=>{if(event.workspaceId!==workspace?.id||event.chatId!==selectedChatId||event.turnId!==voiceTurnRef.current||voiceState!=='speaking')return;if(event.result==='failed'){setVoiceSessionActive(false);setVoiceState('off');setVoicePartial('');setError('Local speech playback failed. Open Settings for voice diagnostics.');return}if(event.result==='completed'&&voiceMode==='hands_free'&&voiceSessionActive){void startVoiceCapture();return}setVoiceState('off');setVoicePartial('')}),[workspace,selectedChatId,voiceMode,voiceDevice,voiceSessionActive,voiceState]);
  useEffect(()=>{if(voiceState!=='thinking'||!workspace||!selectedChatId)return;const voice=voiceRunRef.current;if(!voice||voice.turn!==voiceTurnRef.current||voice.workspaceId!==workspace.id||voice.chatId!==selectedChatId||!voice.runId||voice.spoken)return;const run=runs.find((item)=>String(item.id)===voice.runId);if(!run)return;if(['failed','timed_out','canceled'].includes(String(run.status))){setVoiceSessionActive(false);setVoiceState('off');setVoicePartial('');setError(`Voice turn ${String(run.status).replace('_',' ')}; no stale response will be spoken.`);return}if(run.status!=='completed'||typeof run.assistantMessageId!=='string')return;const chat=chats.find((item)=>item.id===selectedChatId),answer=chat?.messages.find((item)=>item.id===run.assistantMessageId&&item.role==='assistant')?.body;if(!answer?.trim())return;voice.spoken=true;const turn=voice.turn;void window.waypoint.speakVoice(workspace.id,selectedChatId,turn,answer).then(()=>{if(turn===voiceTurnRef.current)setVoiceState('speaking')}).catch((reason)=>{if(turn===voiceTurnRef.current){setVoiceSessionActive(false);setVoiceState('off');setVoicePartial('');showError(reason)}})},[voiceState,workspace,selectedChatId,chats,runs]);
  useEffect(()=>{const prior=voiceScopeRef.current,next={workspaceId:workspace?.id,chatId:selectedChatId},changed=Boolean(prior.workspaceId&&(prior.workspaceId!==next.workspaceId||prior.chatId!==next.chatId));voiceScopeRef.current=next;if(changed){voiceTurnRef.current++;voiceCaptureTargetRef.current=undefined;voiceStateRef.current='off';setVoiceSessionActive(false);setVoiceState('off');setVoicePartial('');void voiceCaptureRef.current.cancel();if(prior.workspaceId&&prior.chatId)void window.waypoint.stopVoice(prior.workspaceId,prior.chatId).catch(()=>undefined)}},[workspace,selectedChatId]);
  useEffect(() => {
  ...
  async function saveVoicePreferences(nextMode=voiceMode,nextDevice=voiceDevice){if(!workspace)return;const value=await window.waypoint.updateVoicePreferences(workspace.id,{mode:nextMode,microphoneId:nextDevice,outputVoice:'system'});setVoiceMode(value.mode);setVoiceDevice(value.microphoneId);setNotice('Voice preferences saved for this workspace on this device.')}
          ...
          <aside ref={overlayRef} className="right-drawer" role="dialog" aria-modal="true" aria-labelledby="drawer-title">
            ...
            {drawer === 'settings' && (
              <div className="drawer-body">
                ...
                <section>
                  <h3>Voice chat</h3>
                  <p className="drawer-intro">Local, turn-based voice with bundled Whisper transcription and the macOS system voice. Microphone audio is ephemeral and never enters chat, backup, sync, or activity.</p>
                  ...
                  <label className="settings-field">Reply voice<select aria-label="Voice output" value="system" disabled><option value="system">macOS system voice</option></select></label>
