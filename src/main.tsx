            ...
            {drawer === 'settings' && (
              <div className="drawer-body">
                ...
                <section>
                  ...
                  {voiceEngineStatus?.engines.map((engine)=><div className={`automation-boundary ${engine.ready?'':'warning'}`} role="status" key={engine.id}><strong>{engine.label} · {engine.ready?'ready':'unavailable'}</strong><span>{engine.reason}</span><small>{engine.conversationOwner==='waypoint-providers'?'Uses the selected Waypoint Codex, Claude, or OpenRouter text route.':'Owns its experimental local conversation; Waypoint provider tools are not available.'}{engine.packageBytes?` · ${(engine.packageBytes/1024/1024).toFixed(1)} MB`:''}</small><small>First audio {engine.metrics.firstAudioMs==null?'not measured':`${engine.metrics.firstAudioMs} ms${engine.metrics.fixture?' fixture':''}`} · interruption {engine.metrics.interruptionMs==null?'not measured':`${engine.metrics.interruptionMs} ms`} · turn end {engine.metrics.turnEndMs==null?'not measured':`${engine.metrics.turnEndMs} ms`}</small></div>)}
