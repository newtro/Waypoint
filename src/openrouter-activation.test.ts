...
const base={enabled:false,liveRequestsEnabled:false,strategicModel:'',everydayModel:'',fallbackProvider:'codex'}
describe('nextOpenRouterActivation',()=>{
 it('requires a protected key and enables both paid-request gates in one explicit action',()=>{expect(()=>nextOpenRouterActivation(base,false)).toThrow('protected storage');expect(nextOpenRouterActivation(base,true)).toEqual({...base,enabled:true,liveRequestsEnabled:true,strategicModel:'moonshotai/kimi-k3',everydayModel:'deepseek/deepseek-v4-flash'})})
